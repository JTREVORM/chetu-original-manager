...
const signIn = async (page, who) => {
  ...
  await page.evaluate(() =>
    Array.from(document.querySelectorAll("button"))
      .find((b) => /sign in/i.test(b.innerText))
      ?.click(),
  );
...
const capture = async (label, viewport, who, pages, suffix) => {
  ...
  for (const [stem, path, proof, search] of pages) {
    ...
    if (search) {
      await page.evaluate(() =>
        Array.from(document.querySelectorAll("button"))
          .find((b) => /^\s*Search\s*$/.test(b.innerText))
          ?.click(),
      );
      await wait(1800);
    }
...
await capture(
  ...
  { width: 1440, height: 950, deviceScaleFactor: 2 },
  ADMIN,
  PAGES,
  "desktop",
);
...
await capture(
  "desktop / loan officer",
  { width: 1440, height: 950, deviceScaleFactor: 2 },
  OFFICER,
  [
    ["dashboard-officer", "/", "Good", false],
    ["member-list-officer", "/member-list", "Member List", true],
  ],
  "desktop",
);
await capture(
...
// The sidebar and the notification drawer, opened rather than navigated to.
{
  ...
  await page
    .waitForFunction(() => document.body.innerText.includes("Member List"), {
      timeout: 60000,
      polling: 400,
    })
    .catch(() => {});
  ...
  await page.evaluate(() =>
    Array.from(document.querySelectorAll("button"))
      .find((b) => b.querySelector("svg.lucide-menu"))
      ?.click(),
  );
  await wait(1200);
  await page.evaluate(() =>
    Array.from(document.querySelectorAll("button"))
      .find((b) => /^Members$/.test(b.innerText.trim()))
      ?.click(),
  );
  await wait(900);
  ...
  await page
    .waitForFunction(() => /Good/.test(document.body.innerText), { timeout: 60000, polling: 400 })
    .catch(() => {});
  await wait(2000);
  await page.evaluate(() =>
    Array.from(document.querySelectorAll("button"))
      .find((b) => /notification/i.test(b.getAttribute("aria-label") || ""))
      ?.click(),
  );
  await wait(1400);
...
// The login screen, signed out.
{
  ...
  await desktop.waitForFunction(() => document.querySelectorAll("input").length >= 2, {
    timeout: 90000,
  });
  await wait(1200);
