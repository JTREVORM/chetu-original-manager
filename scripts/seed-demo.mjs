...
const branchA = must(
  ...
  await svc
    .from("branches")
    .insert({
      branch_name: "Iganga Main",
      branch_code: "IGA-01",
      location: "Iganga Town, Main Street",
      phone: "0392000101",
      manager_name: "Sarah Namubiru",
      status: "Active",
    })
    .select()
    .single(),
...
const branchB = must(
  ...
  await svc
    .from("branches")
    .insert({
      branch_name: "Jinja Central",
      branch_code: "JJA-02",
      location: "Jinja, Clive Road",
      phone: "0392000202",
      manager_name: "Peter Wanyama",
      status: "Active",
    })
    .select()
    .single(),
...
const productA = must(
  ...
  await svc
    .from("loan_products")
    .insert({
      product_name: "Umoja Micro Loan",
      description: "Working capital for market traders and small shops.",
      interest_rate: 15,
...
const productB = must(
  ...
  await svc
    .from("loan_products")
    .insert({
      product_name: "Kilimo Agri Loan",
      description: "Seasonal input finance for smallholder farmers.",
      interest_rate: 12,
      interest_type: "Flat Rate",
      processing_fee_percentage: 4,
      penalty_rate: 1,
...
const manager = await makeStaff(
  "Sarah Namubiru",
  "0771000001",
  "sarah.namubiru@chetumf.co.ug",
  "Branch Manager",
  [branchA.id],
);
const officer1 = await makeStaff(
  "Nakabugo Marjorie",
  "0771000002",
  "marjorie.nakabugo@chetumf.co.ug",
  "Loan Officer",
  [branchA.id],
);
const officer2 = await makeStaff(
  "Okello Brian",
  "0771000003",
  "brian.okello@chetumf.co.ug",
  "Loan Officer",
  [branchB.id],
);
const auditor = await makeStaff(
  "Grace Atim",
  "0771000004",
  "grace.atim@chetumf.co.ug",
  "Auditor",
  [],
);
...
const gRejected = await makeGroup(
  ...
  "Rejected",
  {
    rejection_reason: "Only 3 members listed — the minimum is 5.",
    reviewed_by: manager,
  ...
  return must(
    ...
    await svc
      ...
      .insert({
        ...
        occupation: "Trader",
        phone_number: phone,
        physical_address: "Nakalama",
        village: "Nakalama",
        parish: "Nakalama",
        sub_county: "Iganga",
        district: branch.branch_name.split(" ")[0],
        member_type: "Member",
        group_id: group?.id ?? null,
        branch_id: branch.id,
        loan_officer_id: officerId,
        registered_by: officerId,
        date_registered: day(-200),
        approval_status: approval,
        status: "Active",
        ...extra,
      })
...
m.mary = await makeMember(
  ...
  officer1,
  "Approved",
  { reviewed_by: manager, reviewed_at: stamp(-198) },
);
m.pheibe = await makeMember(
  "Mpayaliku Pheibe",
  "Female",
  "0742372465",
  gDembe,
...
m.zeresi = await makeMember(
  ...
  officer1,
  "Approved",
  { reviewed_by: manager, reviewed_at: stamp(-190) },
);
m.jamira = await makeMember(
  "Nalunga Jamira",
  "Female",
  "0750875995",
  gTwezike,
  ...
  officer1,
  "Approved",
  { reviewed_by: manager, reviewed_at: stamp(-170) },
);
m.rebecca = await makeMember(
  "Namajja Rebecca",
  "Female",
  "0702892788",
  gTwezike,
...
m.hadija = await makeMember(
  ...
  officer1,
  "Approved",
  { reviewed_by: manager, reviewed_at: stamp(-160) },
);
m.moses = await makeMember(
  "Waiswa Moses",
  "Male",
  "0781223344",
  gKilimo,
  ...
  officer2,
  "Approved",
  { reviewed_by: manager, reviewed_at: stamp(-140) },
);
m.esther = await makeMember(
  "Akello Esther",
  "Female",
  "0781223355",
  gKilimo,
...
m.rejected = await makeMember(
  ...
  "Rejected",
  {
    rejection_reason: "National ID photo unreadable — re-capture and resubmit.",
    reviewed_by: manager,
    reviewed_at: stamp(-15),
  },
);
m.inactive = await makeMember(
  "Kagoya Betty",
  "Female",
  "0788990044",
  gDembe,
  ...
  {
    status: "Inactive",
    inactive_reason: "Relocated out of the branch catchment",
    inactive_date: day(-45),
  },
);
m.deceased = await makeMember(
  "Mukisa Joseph",
  "Male",
  "0788990055",
  gTwezike,
...
const feeRows = Object.values(m)
  ...
  .map((c) => ({
    ...
    total_amount: 10000,
    payment_method: "Cash",
    receipt_number: `CM-ADM-${c.client_number}`,
    branch_id: c.branch_id,
    collected_by: c.loan_officer_id,
  }));
...
const buildSchedule = (loanId, weeks, weekly, firstDue, paidWeeks) => {
  ...
  for (let w = 1; w <= weeks; w += 1) {
    ...
    rows.push({
      ...
      principal_portion: Math.round(weekly * 0.87),
      interest_portion: weekly - Math.round(weekly * 0.87),
      paid_amount: paid,
      remaining_balance: weekly - paid,
  ...
  const app = must(
    ...
    await svc
      ...
      .insert({
        ...
        guarantor_name: "Nalunga Jamira",
        guarantor_phone: "0750875995",
        guarantor_relationship: "Group Member",
        guarantor_nin: "CF83075103ZAJD",
        guarantor_address: "Nakalama",
        status: opts.appStatus,
        submitted_by: client.loan_officer_id,
        reviewed_by: opts.appStatus === "Pending" ? null : manager,
  ...
  const loan = must(
    ...
    await svc
      ...
      .insert({
        ...
        loan_period_weeks: weeks,
        total_interest_amount: interest,
        total_amount_payable: total,
        weekly_installment: weekly,
        processing_fee_amount: proc,
        crb_fee_amount: crb,
        security_amount: sec,
        security_balance: opts.securityBalance ?? sec,
        group_maintenance_fee: FEES.gm,
        net_disbursed_amount: principal - deductions,
        first_repayment_date: opts.firstDue,
        final_due_date: day(opts.finalDueOffset ?? 120),
  ...
  if (opts.loanStatus !== "Pending") {
    must(
      ...
      await svc
        ...
        .insert(buildSchedule(loan.id, weeks, weekly, opts.firstDue, paidWeeks))
        .select(),
    );
      ...
      must(
        ...
        await svc
          ...
          .insert({
            ...
            client_id: client.id,
            amount_paid: weekly,
            payment_date: day((opts.disbursedDays ?? -55) + w * 7),
            payment_method: "Cash",
            collection_type: "Regular",
            recorded_by: client.loan_officer_id,
          })
          .select()
          .single(),
      );
...
await makeLoan(m.mary, productA, 1000000, 20, {
  loanStatus: "Partially Paid",
  appStatus: "Disbursed",
  firstDue: day(-48),
  paidWeeks: 6,
});
await makeLoan(m.pheibe, productA, 800000, 20, {
  loanStatus: "Overdue",
  appStatus: "Disbursed",
  firstDue: day(-75),
  paidWeeks: 2,
...
await makeLoan(m.zeresi, productA, 600000, 16, {
  loanStatus: "Active",
  appStatus: "Disbursed",
  firstDue: day(-14),
  paidWeeks: 2,
});
await makeLoan(m.jamira, productA, 1500000, 24, {
  loanStatus: "Partially Paid",
  appStatus: "Disbursed",
  firstDue: day(-35),
  paidWeeks: 5,
});
await makeLoan(m.moses, productB, 2000000, 30, {
  loanStatus: "Active",
  appStatus: "Disbursed",
  firstDue: day(-21),
  paidWeeks: 3,
  purpose: "Agriculture - Crop Farming",
});
await makeLoan(m.esther, productB, 1200000, 24, {
  ...
  extra: {
    is_bad_debt: true,
    bad_debt_declared_at: stamp(-10),
    bad_debt_comment: "Member untraceable since March.",
    writeoff_status: "Declared",
  },
});
await makeLoan(m.rebecca, productA, 500000, 12, {
  loanStatus: "Fully Paid",
  appStatus: "Disbursed",
  firstDue: day(-120),
  paidWeeks: 12,
  disbursedDays: -127,
  outstandingOverride: 0,
});
// Approved, cash not yet released — populates Waiting for Disburse.
await makeLoan(m.hadija, productA, 700000, 16, {
  loanStatus: "Pending",
  appStatus: "Approved",
  firstDue: day(7),
  appDays: -3,
...
// Turned down.
await makeLoan(m.zeresi, productA, 900000, 20, {
  appStatus: "Rejected",
  appDays: -8,
    ...
    .insert([
      {
        ...
        reference_number: "STB-CAP-001",
        transaction_date: day(-200),
        branch_id: branchA.id,
      },
      {
        transaction_number: "",
        transaction_type: "Deposit",
        category: "Branch Float",
        description: "Float to Jinja Central",
        amount: 8000000,
        balance_after: 33000000,
        reference_number: "STB-FLT-002",
        transaction_date: day(-150),
    ...
    .insert([
      {
        expense_number: "",
        category: "Rent",
        description: "Iganga branch rent — quarter",
        amount: 1800000,
        expense_date: day(-40),
      ...
      },
      {
        expense_number: "",
        category: "Fuel",
        description: "Field collection fuel",
        amount: 320000,
        expense_date: day(-12),
        payment_method: "Cash",
        branch_id: branchA.id,
      },
      {
        expense_number: "",
        category: "Salaries",
        description: "Field staff salaries",
        amount: 4200000,
        expense_date: day(-5),
        payment_method: "Bank Transfer",
        branch_id: branchA.id,
...
const transfer = must(
  ...
  await svc
    ...
    .insert({
      ...
      client_id: m.zeresi.id,
      from_branch_id: branchA.id,
      to_branch_id: branchB.id,
      from_group_id: gDembe.id,
      status: "Pending",
      reason: "Member has relocated to Jinja.",
...
const admin = must(
  "admin",
  await svc.from("profiles").select("id").eq("role", "Administrator").limit(1).single(),
);
const notes = [
  [
    "Group waiting for approval",
    "Bugiri Youth Group (CM-GRP-2026-0004) was created by Nakabugo Marjorie and needs your approval.",
    "Application",
  ...
  [
    ...
    "Nabirye Justine was admitted by Nakabugo Marjorie and needs your approval.",
    "Application",
  ...
  [
    ...
    "Nabirye Justine applied for UGX 400,000.",
    "Application",
  ...
  [
    ...
    "Mirembe Hadija was approved. The loan is ready for disbursement.",
    "Disbursement",
  ...
  [
    "Repayment received",
    "UGX 57,500 received on loan from Nangobi Mary.",
    "Repayment",
    "/repayments",
    8,
    true,
  ...
  [
    "Member transfer awaiting receipt",
    "Bagozanya Zeresi has been sent to Jinja Central and is waiting to be received.",
    "Alert",
    "/transfers/receive",
    26,
    true,
