...
const Fact: React.FC<{ label: string; value: React.ReactNode; strong?: boolean }> = ({
  label,
  value,
  strong,
}) => (
  <div>
    ...
    <dd
      className={`mt-0.5 text-[13px] ${strong ? "font-black text-slate-900" : "font-semibold text-slate-800"}`}
    >
      {value}
    </dd>
  </div>
    ...
    <section className={`overflow-hidden rounded-lg border bg-white shadow-xs ${tone.ring}`}>
      <header className={`flex flex-wrap items-start justify-between gap-4 px-5 py-4 ${tone.head}`}>
        <div className="flex items-start gap-3">
          <span
            className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white ${tone.text}`}
          >
            {isOpen ? (
          ...
          <div>
            <p
              className={`flex items-center gap-2 text-[13px] font-black tracking-wide ${tone.text}`}
            >
            ...
            <p className="mt-0.5 text-[12px] font-semibold text-slate-600">
              {longDate(businessDate)} · {branchName}
            </p>
          </div>
        </div>
      ...
      <dl className="grid grid-cols-2 gap-x-4 gap-y-3 px-5 py-4 sm:grid-cols-4">
        {isClosed ? (
          <>
            ...
            <Fact
              label="Officers submitted"
              value={`${stats.officersSubmitted} of ${stats.officersTotal}`}
            />
          </>
        ) : isOpen ? (
          <>
