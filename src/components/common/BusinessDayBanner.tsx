...
const Tile: React.FC<{
  ...
  <div className="rounded border border-white/15 bg-white/10 px-3 py-2">
    ...
    <p
      className={`mt-0.5 text-lg font-black ${tone === "amber" ? "text-amber-300" : tone === "red" ? "text-red-300" : "text-white"}`}
    >
      {value}
    </p>
  </div>
...
export const BusinessDayBanner: React.FC = () => {
  ...
  const isDayClosed =
    day?.status === "CLOSED" || day?.status === "APPROVED" || day?.status === "LOCKED";
          ...
          <div className="min-w-0">
            <p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-wide text-white/80">
              <span className="relative flex h-2.5 w-2.5">
                {isDayOpen && (
                  <span
                    className={`absolute inline-flex h-full w-full rounded-full ${tone.dot} opacity-70 motion-safe:animate-ping`}
                  />
                )}
            ...
            <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1 text-[12px] text-white/75">
              {isDayOpen && day?.opened_at && (
                <span>
                  Opened {shortTime(day.opened_at)} ·{" "}
                  <strong className="text-white">
                    {formatDuration(day.opened_at, now)} active
                  </strong>
                </span>
              )}
              {isDayClosed && day?.closed_at && (
                <span>
                  Closed at <strong className="text-white">{shortTime(day.closed_at)}</strong>
                </span>
              )}
              {day?.opened_by && (
                <span>
                  By <strong className="text-white">{staffNames[day.opened_by] || "—"}</strong>
                </span>
              )}
              {isOfficer && (
                <span>
                  My status:{" "}
                  <strong className="text-white">{officerDayStatus.replace(/_/g, " ")}</strong>
                </span>
              )}
              {isOfficer && myDay?.submitted_at && (
                <span>Submitted {shortTime(myDay.submitted_at)}</span>
              )}
            </div>
          </div>
        </div>
          ...
          {isDayClosed ? (
            ...
            <>
              ...
              <Tile
                ...
                tone={pendingRequests ? "amber" : "plain"}
              />
            </>
          )}
        </div>
