...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (isSupabaseConfigured) {
      const syncProfile = async (sessionUser: NonNullable<Awaited<ReturnType<typeof supabase.auth.getSession>>['data']['session']>['user']) => {
        ...
        const { data: created } = await supabase
          ...
          .insert({
            ...
            full_name: sessionUser.user_metadata?.full_name || sessionUser.email?.split('@')[0] || 'System User',
            role: fallbackRole,
    ...
    try {
      ...
      if (isSupabaseConfigured) {
        ...
        if (cleanPhone.includes('@')) {
          ...
          const { data: resolved } = await supabase.rpc('account_email_for_phone', { _phone: cleanPhone });
