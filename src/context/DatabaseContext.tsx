...
interface DatabaseContextType {
  ...
  addClient: (client: Omit<Client, 'id' | 'client_number' | 'created_at' | 'approval_status'>) => Promise<Client>;
  ...
  addClientGroup: (group: Omit<ClientGroup, 'id' | 'group_code' | 'member_count' | 'created_at' | 'approval_status'>) => Promise<ClientGroup>;
  ...
  submitLoanApplication: (app: Omit<LoanApplication, 'id' | 'application_number' | 'status' | 'created_at'>) => Promise<LoanApplication>;
  ...
  recordRepayment: (loanId: string, amount: number, method: PaymentMethod, notes?: string) => Promise<LoanRepayment>;
  settleLoan: (loanId: string, amount: number, method: PaymentMethod, notes?: string) => Promise<LoanRepayment>;
  ...
  receiveMemberTransfer: (transferId: string, toGroupId?: string, toOfficerId?: string) => Promise<void>;
  ...
  addSavingsTransaction: (accountId: string, amount: number, type: SavingsTransactionType, method: PaymentMethod, notes?: string) => Promise<SavingsTransaction>;
  recordGroupAttendance: (groupId: string, meetingDate: string, attendees: string[], notes?: string) => Promise<GroupAttendance>;
  addExpense: (expense: Omit<Expense, 'id' | 'expense_number' | 'created_at'>) => Promise<Expense>;
  addBankTransaction: (tx: Omit<BankTransaction, 'id' | 'transaction_number' | 'balance_after' | 'created_at'>) => Promise<BankTransaction>;
...
export const DatabaseProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // The initial mount happens before Supabase has restored the session, so
  // RLS-protected tables (client_groups, loans, …) would come back empty and
  // stay empty until a manual reload. Refetch whenever the signed-in user
  // changes so the workspace fills in as soon as the session exists.
  useEffect(() => {
    refetch();
  }, [user?.id]);

  // Keep the workspace live: quietly re-pull every 30s (no spinner, no reload)
  // while a user is signed in and the tab is visible.
  useEffect(() => {
    if (!user?.id) return;
  ...
  const logAudit = async (action: string, module: string, details: string, record_id?: string) => {
    ...
    if (isSupabaseConfigured) {
      ...
      if (!sessionData.session) {
        setAuditLogs(prev => [{ ...insertPayload, id: `local-${Date.now()}`, created_at: new Date().toISOString() } as unknown as AuditLog, ...prev]);
        return;
      }
      ...
      const localLog: AuditLog = { ...insertPayload, id: `log-${Date.now()}`, created_at: new Date().toISOString() };
  ...
  const visibleClients = institutionWide
    ...
    : clients.filter(c =>
        branchMatches(c.branch_id) && (isBranchManager || c.registered_by === user?.id));
  ...
  const visibleGroups = institutionWide
    ...
    : clientGroups.filter(g =>
        branchMatches(g.branch_id) && (isBranchManager || g.created_by === user?.id || g.loan_officer_id === user?.id));
  ...
  const visibleSavingsAccounts = institutionWide
    ? savingsAccounts.filter(s => !selectedBranchId || (s.client_id && visibleClientIds.has(s.client_id)) || (s.group_id && visibleGroupIds.has(s.group_id)))
    : savingsAccounts.filter(s => (s.client_id && visibleClientIds.has(s.client_id)) || (s.group_id && visibleGroupIds.has(s.group_id)));
  const visibleSavingsAccountIds = new Set(visibleSavingsAccounts.map(s => s.id));
  const visibleSavingsTransactions = savingsTransactions.filter(t => visibleSavingsAccountIds.has(t.account_id));
  ...
  const totalRepaymentsCollected = visibleRepayments.reduce((sum, r) => sum + Number(r.amount_paid), 0);
  ...
  const currentBankBalance = visibleBankTransactions.reduce(
    (balance, transaction) => balance + (transaction.transaction_type === 'Deposit' ? Number(transaction.amount) : -Number(transaction.amount)),
    0
  );
  ...
  const addBranch = async (branchData: Omit<Branch, 'id' | 'created_at'>): Promise<Branch> => {
    ...
    const { data, error } = await supabase.from('branches').insert([nullifyBlanks({ ...branchData })]).select().single();
  ...
  const addClient = async (clientData: Omit<Client, 'id' | 'client_number' | 'created_at' | 'approval_status'>): Promise<Client> => {
    ...
    const approval_status: Client['approval_status'] = role === 'Loan Officer' ? 'Pending' : 'Approved';
    ...
    const { data: savData, error: savError } = await supabase.from('savings_accounts').insert([savInsertPayload]).select().single();
    if (!savError && savData) {
      setSavingsAccounts(prev => [{ ...savData, client: newClient } as SavingsAccount, ...prev]);
    }

    await logAudit('Client Registration', 'Client Management', `Registered client ${newClient.full_name} (${newClient.client_number}) and created Savings Account (${savData?.account_number || '—'})`, newClient.client_number);
  ...
  const updateClient = async (id: string, clientData: Partial<Client>) => {
    requireRoles(['Administrator', 'Branch Manager', 'Loan Officer']);
    if (!isSupabaseConfigured) return;
    const { error } = await supabase.from('clients').update(nullifyBlanks({ ...clientData, updated_at: new Date().toISOString() })).eq('id', id);
    if (!error) {
      setClients(prev => prev.map(c => c.id === id ? { ...c, ...clientData, updated_at: new Date().toISOString() } : c));
    }
    await logAudit('Update Client Profile', 'Client Management', `Updated profile information for client ID ${id}`, id);
  };
  ...
  const approveClient = async (id: string) => {
    ...
    const { error } = await supabase
      .from('clients')
      .update({ approval_status: 'Approved', rejection_reason: null, reviewed_by: user?.id, reviewed_at: now, updated_at: now })
      .eq('id', id);
    if (error) throw new Error(error.message);
    setClients(prev => prev.map(c => c.id === id ? { ...c, approval_status: 'Approved', rejection_reason: undefined, reviewed_by: user?.id, reviewed_at: now, updated_at: now } : c));
    const client = clients.find(c => c.id === id);
    await logAudit('Approved Member', 'Member Management', `Approved member ${client?.full_name || id}`, id);
    await sendNotification({
  ...
  const rejectClient = async (id: string, reason: string) => {
    ...
    const { error } = await supabase
      .from('clients')
      .update({ approval_status: 'Rejected', rejection_reason: reason, reviewed_by: user?.id, reviewed_at: now, updated_at: now })
      .eq('id', id);
    if (error) throw new Error(error.message);
    setClients(prev => prev.map(c => c.id === id ? { ...c, approval_status: 'Rejected', rejection_reason: reason, reviewed_by: user?.id, reviewed_at: now, updated_at: now } : c));
    const client = clients.find(c => c.id === id);
    await logAudit('Rejected Member', 'Member Management', `Rejected member ${client?.full_name || id}: ${reason}`, id);
    await sendNotification({
  ...
  const declareClientDeath = async (id: string, deathDate: string) => {
    ...
    const client = clients.find(c => c.id === id);
    await logAudit('Declared Member Deceased', 'Member Management', `Recorded death of member ${client?.full_name || id}`, id);
    await sendNotification({
  ...
  const addClientGroup = async (groupData: Omit<ClientGroup, 'id' | 'group_code' | 'member_count' | 'created_at' | 'approval_status'>): Promise<ClientGroup> => {
    requireRoles(['Administrator', 'Branch Manager', 'Loan Officer']);
    if (!isSupabaseConfigured) throw new Error('Database not configured');
    // Loan Officers submit for Branch Manager review; management creates live.
    const approval_status: ClientGroup['approval_status'] = role === 'Loan Officer' ? 'Pending' : 'Approved';
    ...
    const { data, error } = await supabase.from('client_groups').insert([newGroupRow]).select().single();
    if (error || !data) {
    ...
    await logAudit('Created Client Group', 'Client Groups', `Created group ${newGroup.group_name} (${newGroup.group_code})`, newGroup.group_code);
    await sendNotification(
  ...
  const updateClientGroup = async (id: string, groupData: Partial<ClientGroup>) => {
    requireRoles(['Administrator', 'Branch Manager', 'Loan Officer']);
    if (!isSupabaseConfigured) return;
    const { error } = await supabase.from('client_groups').update({ ...groupData, updated_at: new Date().toISOString() }).eq('id', id);
    if (!error) {
      setClientGroups(prev => prev.map(g => g.id === id ? { ...g, ...groupData, updated_at: new Date().toISOString() } : g));
    }
  ...
  const approveClientGroup = async (id: string) => {
    ...
    const { error } = await supabase
      ...
      .eq('id', id);
    if (error) throw new Error(error.message);
    setClientGroups(prev => prev.map(g => g.id === id ? { ...g, approval_status: 'Approved', rejection_reason: undefined, reviewed_by: user?.id, reviewed_at: now, updated_at: now } : g));
    const group = clientGroups.find(g => g.id === id);
    await logAudit('Approved Client Group', 'Client Groups', `Approved group ${group?.group_name || id}`, id);
    await sendNotification({
  ...
  const rejectClientGroup = async (id: string, reason: string) => {
    ...
    const { error } = await supabase
      ...
      .eq('id', id);
    if (error) throw new Error(error.message);
    setClientGroups(prev => prev.map(g => g.id === id ? { ...g, approval_status: 'Rejected', rejection_reason: reason, reviewed_by: user?.id, reviewed_at: now, updated_at: now } : g));
    const group = clientGroups.find(g => g.id === id);
    await logAudit('Rejected Client Group', 'Client Groups', `Rejected group ${group?.group_name || id}: ${reason}`, id);
    await sendNotification({
  ...
  const deleteClientGroup = async (id: string) => {
    ...
    if (!error) {
      ...
      setClients(prev => prev.map(c => c.group_id === id ? { ...c, group_id: undefined } : c));
    }
    await logAudit('Deleted Client Group', 'Client Groups', `Removed group ID ${id}`, id);
  };

  const addLoanProduct = async (prodData: Omit<LoanProduct, 'id' | 'created_at'>): Promise<LoanProduct> => {
    requireRoles(['Administrator']);
    if (!isSupabaseConfigured) throw new Error('Database not configured');
    const { data, error } = await supabase.from('loan_products').insert([prodData]).select().single();
    ...
    await logAudit('Created Loan Product', 'Loan Products', `Created product ${newProd.product_name} (${newProd.interest_rate}%)`, newProd.id);
    return newProd;
  };
  ...
  const updateLoanProduct = async (id: string, productData: Partial<LoanProduct>) => {
    ...
    await logAudit('Updated Loan Product', 'Loan Products', `Modified product settings for product ID ${id}`, id);
  };

  const submitLoanApplication = async (appData: Omit<LoanApplication, 'id' | 'application_number' | 'status' | 'created_at'>): Promise<LoanApplication> => {
    ...
    const { data, error } = await supabase.from('loan_applications').insert([newAppRow]).select().single();
    if (error || !data) throw new Error(error?.message || 'Failed to submit application');
    const newApp: LoanApplication = { ...data, client: clientObj, product: prodObj } as LoanApplication;
    const application_number = newApp.application_number;
    setLoanApplications(prev => [newApp, ...prev]);
    await logAudit('Submitted Loan Application', 'Loan Applications', `Submitted application ${application_number} for UGX ${newApp.requested_amount}`, application_number);
    await sendNotification({
  ...
  const approveLoanApplication = async (appId: string): Promise<Loan> => {
    ...
    if (!product) throw new Error('Loan product not found. Please create a loan product before approving applications.');
    const client = clients.find(c => c.id === app.client_id);

    const { error: appError } = await supabase.from('loan_applications').update({ status: 'Approved', reviewed_by: user?.id, updated_at: new Date().toISOString() }).eq('id', appId);
    if (appError) throw new Error(appError.message);
    setLoanApplications(prev => prev.map(a => a.id === appId ? { ...a, status: 'Approved', reviewed_by: user?.id } : a));
    ...
    const { data: loanData, error: loanError } = await supabase.from('loans').insert([newLoanRow]).select().single();
    ...
    const { error: schedError } = await supabase.from('loan_repayment_schedule').insert(scheduleRows);
    if (schedError) {
    ...
    await logAudit('Approved Loan Application', 'Loan Approvals', `Approved application ${app.application_number} -> Generated Loan ${loan_number}`, loan_number);
    await sendNotification({
  ...
  const rejectLoanApplication = async (appId: string, reason: string) => {
    requireRoles(['Administrator', 'Branch Manager']);
    if (!isSupabaseConfigured) return;
    const { error } = await supabase.from('loan_applications').update({ status: 'Rejected', rejection_reason: reason, reviewed_by: user?.id, updated_at: new Date().toISOString() }).eq('id', appId);
    if (!error) {
      setLoanApplications(prev => prev.map(a => a.id === appId ? { ...a, status: 'Rejected', rejection_reason: reason, reviewed_by: user?.id } : a));
    }
    await logAudit('Rejected Loan Application', 'Loan Approvals', `Rejected application ID ${appId}: ${reason}`, appId);
  ...
  const resubmitLoanApplication = async (appId: string) => {
    ...
    const app = loanApplications.find(a => a.id === appId);
    if (!app) throw new Error('Loan application not found');
    if (app.status !== 'Rejected') throw new Error('Only a rejected application can be resubmitted');
    ...
    const { error } = await supabase
      ...
      .update({ status: 'Pending', rejection_reason: null, reviewed_by: null, updated_at: new Date().toISOString() })
      .eq('id', appId);
    if (error) throw new Error(error.message);
    setLoanApplications(prev => prev.map(a =>
      a.id === appId ? { ...a, status: 'Pending', rejection_reason: undefined, reviewed_by: undefined } : a));

    await logAudit('Resubmitted Loan Application', 'Loan Applications', `Resubmitted application ${app.application_number} for approval`, app.application_number);
    await sendNotification({
  ...
  const disburseLoan = async (loanId: string) => {
    ...
    const now = new Date().toISOString();
    const { error: loanError } = await supabase.from('loans').update({ status: 'Active', disbursed_by: user?.id, disbursed_at: now, updated_at: now }).eq('id', loanId);
    if (!loanError) {
      setLoans(prev => prev.map(l => l.id === loanId ? { ...l, status: 'Active', disbursed_by: user?.id, disbursed_at: now } : l));
    }

    const { error: appError } = await supabase.from('loan_applications').update({ status: 'Disbursed', updated_at: now }).eq('id', targetLoan.application_id);
    if (!appError) {
      setLoanApplications(prev => prev.map(a => a.id === targetLoan.application_id ? { ...a, status: 'Disbursed' } : a));
    }
    ...
    const branchLedgerBalance = bankTransactions
      ...
      .reduce((balance, transaction) => balance + (transaction.transaction_type === 'Deposit' ? Number(transaction.amount) : -Number(transaction.amount)), 0);
    ...
    const { data: btData, error: txError } = await supabase.from('bank_transactions').insert([btInsertPayload]).select().single();
    if (!txError && btData) {
      setBankTransactions(prev => [btData as BankTransaction, ...prev]);
    }
    await logAudit('Disbursed Loan', 'Loan Disbursement', `Disbursed loan ${targetLoan.loan_number} (Net Amount: UGX ${targetLoan.principal_amount})`, targetLoan.loan_number);
    await sendNotification({
  ...
  const recordRepayment = async (
    ...
    const completionPct = Math.min(100, Math.round(((targetLoan.total_amount_payable - newOutstanding) / targetLoan.total_amount_payable) * 100));
    ...
    const { data: repData, error: repError } = await supabase.from('loan_repayments').insert([repInsertPayload]).select().single();
    if (repError || !repData) throw new Error(repError?.message || 'Could not record the repayment');
    const newRepayment: LoanRepayment = { ...repData, loan: targetLoan, client: targetLoan.client } as LoanRepayment;
    ...
    await logAudit('Recorded Weekly Repayment', 'Weekly Repayments', `Recorded payment of UGX ${amount} for loan ${targetLoan.loan_number} (Receipt #${receipt_number})`, receipt_number);
    await sendNotification({
  ...
  const settleLoan = async (
    ...
    const targetLoan = loans.find(l => l.id === loanId);
    if (!targetLoan) throw new Error('Loan not found');
    if (CLOSED_LOAN_STATUSES.includes(targetLoan.status)) throw new Error('This loan is already closed');
    ...
    const { data: repData, error: repError } = await supabase.from('loan_repayments').insert([repInsertPayload]).select().single();
    if (repError || !repData) throw new Error(repError?.message || 'Failed to record the settlement');
    const newRepayment: LoanRepayment = { ...repData, loan: targetLoan, client: targetLoan.client } as LoanRepayment;
    setRepayments(prev => [newRepayment, ...prev]);
    ...
    await logAudit('Settled Loan', 'Loan Settlement', `Settled loan ${targetLoan.loan_number} with UGX ${amount} (Receipt #${newRepayment.receipt_number})`, newRepayment.receipt_number);
    await sendNotification({
      title: 'Loan settled',
  ...
  const writeOffLoan = async (loanId: string, reason: string) => {
    ...
    const targetLoan = loans.find(l => l.id === loanId);
    if (!targetLoan) throw new Error('Loan not found');
    if (!targetLoan.is_bad_debt) throw new Error('Declare the loan a bad debt before writing it off');
    ...
    await supabase.from('bad_loan_comments').insert({ loan_id: loanId, comment: `Written off: ${reason}` });

    await logAudit('Wrote Off Loan', 'Loan Writeoff', `Wrote off loan ${targetLoan.loan_number} (UGX ${writtenOff}): ${reason}`, targetLoan.loan_number);
    await sendNotification({
  ...
  const undoDisbursement = async (loanId: string, reason: string) => {
    ...
    const targetLoan = loans.find(l => l.id === loanId);
    if (!targetLoan) throw new Error('Loan not found');
    if (targetLoan.status !== 'Active') throw new Error('Only an active, undisbursed-in-error loan can be rolled back');
    ...
    setLoanApplications(prev => prev.map(a =>
      a.id === targetLoan.application_id ? { ...a, status: 'Approved' } : a));
    ...
    const branchLedgerBalance = bankTransactions
      ...
      .reduce((bal, t) => bal + (t.transaction_type === 'Deposit' ? Number(t.amount) : -Number(t.amount)), 0);
    const { data: btData } = await supabase.from('bank_transactions').insert([{
      transaction_number: txNum,
    ...
    await logAudit('Undid Loan Disbursement', 'Loan Rollback', `Rolled back disbursement of loan ${targetLoan.loan_number}: ${reason}`, targetLoan.loan_number);
    await sendNotification({
  ...
  const undoRepayment = async (repaymentId: string, reason: string) => {
    ...
    const { error: delError } = await supabase.from('loan_repayments').delete().eq('id', repaymentId);
    ...
    const rebuiltSchedule = (targetLoan.schedule || [])
      ...
      .map(row => {
        ...
        if (toAllocate >= due) {
          toAllocate -= due;
          return { ...row, paid_amount: due, remaining_balance: 0, status: 'Paid' as RepaymentStatus };
        }
    ...
    const completionPct = total > 0 ? Math.min(100, Math.round(((total - newOutstanding) / total) * 100)) : 0;
    const restoredStatus: LoanStatus = newOutstanding === 0
      ? 'Fully Paid'
      : survivingTotal > 0 ? 'Partially Paid' : 'Active';
    ...
    setLoans(prev => prev.map(l => l.id === target.loan_id ? {
      ...l,
      outstanding_balance: newOutstanding,
      completion_percentage: completionPct,
      status: restoredStatus,
    ...
    await logAudit('Undid Repayment', 'Loan Rollback', `Reversed receipt ${target.receipt_number} of UGX ${target.amount_paid} on loan ${targetLoan.loan_number}: ${reason}`, target.receipt_number);
    await sendNotification({
  ...
  const requestMemberTransfer = async (input: {
    ...
    if (client.branch_id === input.toBranchId) throw new Error('The member is already in that branch');
    if (client.approval_status !== 'Approved') throw new Error('Only an approved member can be transferred');
    ...
    await logAudit('Requested Member Transfer', 'Transfers', `Requested transfer of ${client.full_name} (${client.client_number}) to ${toBranch?.branch_name || 'another branch'}`, created.id);
    await sendNotification({
   ...
   */
  const receiveMemberTransfer = async (transferId: string, toGroupId?: string, toOfficerId?: string) => {
    requireRoles(['Administrator', 'Branch Manager']);
    if (!isSupabaseConfigured) throw new Error('Database not configured');
    ...
    await logAudit('Received Member Transfer', 'Transfers', `Received ${client?.full_name || 'a member'} into this branch`, transferId);
    await sendNotification({
  ...
  const rejectMemberTransfer = async (transferId: string, reason: string) => {
    ...
    setTransfers(prev => prev.map(t => t.id === transferId
      ? { ...t, status: 'Rejected', rejection_reason: reason.trim(), actioned_by: user?.id, actioned_at: new Date().toISOString() }
      : t));

    await logAudit('Rejected Member Transfer', 'Transfers', `Rejected transfer of ${client?.full_name || 'a member'}: ${reason}`, transferId);
    await sendNotification({
  ...
  const cancelMemberTransfer = async (transferId: string) => {
    ...
    setTransfers(prev => prev.map(t => t.id === transferId
      ? { ...t, status: 'Rejected', rejection_reason: 'Withdrawn by the sending branch', actioned_by: user?.id, actioned_at: now }
      : t));
    await logAudit('Withdrew Member Transfer', 'Transfers', `Withdrew transfer ${transferId}`, transferId);
    await sendNotification({
  ...
  const transferMemberGroup = async (clientId: string, toGroupId: string, reason?: string) => {
    ...
    await refetch({ silent: true });
    await logAudit('Interchanged Member Group', 'Transfers', `Moved ${client?.full_name || 'a member'} into group ${group?.group_name || toGroupId}`, clientId);
    await sendNotification({
  ...
  const transferGroupOfficer = async (groupId: string, toOfficerId: string, reason?: string) => {
    ...
    await refetch({ silent: true });
    await logAudit('Transferred Group Officer', 'Transfers', `Reassigned group ${group?.group_name || groupId} to a new loan officer`, groupId);
    await sendNotification({
  ...
  const addSavingsTransaction = async (
    ...
    const newBalance = type === 'Withdrawal'
      ? Math.max(0, acc.balance - amount)
      : acc.balance + amount;

    const { error: accError } = await supabase.from('savings_accounts').update({ balance: newBalance, updated_at: new Date().toISOString() }).eq('id', accountId);
    if (!accError) {
      setSavingsAccounts(prev => prev.map(a => a.id === accountId ? { ...a, balance: newBalance, updated_at: new Date().toISOString() } : a));
    }
    ...
    const { data: stxData, error: txError } = await supabase.from('savings_transactions').insert([stxInsertPayload]).select().single();
    const newTx: SavingsTransaction = stxData
      ? { ...stxData, account: acc } as SavingsTransaction
      : { ...stxInsertPayload, id: `stx-${Date.now()}`, created_at: new Date().toISOString(), account: acc } as SavingsTransaction;
    if (!txError) {
      setSavingsTransactions(prev => [newTx, ...prev]);
    }
    await logAudit(`Recorded Savings ${type}`, 'Savings Management', `Recorded ${type} of UGX ${amount} for Account ${acc.account_number} (Receipt #${recNum})`, recNum);
    return newTx;
  };
  ...
  const recordGroupAttendance = async (
    ...
    await logAudit('Recorded Group Attendance', 'Client Groups', `Recorded attendance for Group ID ${groupId} on ${meetingDate} (${attendees.length} members present)`, groupId);
    return newAtt;
  };

  const addExpense = async (expData: Omit<Expense, 'id' | 'expense_number' | 'created_at'>): Promise<Expense> => {
    requireRoles(['Administrator']);
    if (!isSupabaseConfigured) throw new Error('Database not configured');
  ...
  const addBankTransaction = async (txData: Omit<BankTransaction, 'id' | 'transaction_number' | 'balance_after' | 'created_at'>): Promise<BankTransaction> => {
    requireRoles(['Administrator']);
    if (!isSupabaseConfigured) throw new Error('Database not configured');
    ...
    const branchLedgerBalance = bankTransactions
      ...
      .reduce((balance, transaction) => balance + (transaction.transaction_type === 'Deposit' ? Number(transaction.amount) : -Number(transaction.amount)), 0);
    const balance_after = txData.transaction_type === 'Deposit'
    ...
    const { data, error } = await supabase.from('bank_transactions').insert([newTxRow]).select().single();
  ...
  const updateSettings = async (newSettings: Partial<SystemSettings>) => {
    requireRoles(['Administrator']);
    if (!isSupabaseConfigured) return;
    const { error } = await supabase.from('settings').update({ ...newSettings, updated_at: new Date().toISOString() }).eq('id', 1);
    if (!error) {
      setSettings(prev => ({ ...prev, ...newSettings, updated_at: new Date().toISOString() }));
    }
    await logAudit('Updated System Settings', 'Settings', 'Modified global system configurations and parameters.');
  };
  ...
  const clearAllData = async () => {
    ...
    await supabase.from('settings').update({ company_name: 'Chetu Microfinance Ltd', default_currency: 'UGX', default_interest_rate: 15.0, default_processing_fee: 2.0, receipt_footer: defaultSettings.receipt_footer, report_header: defaultSettings.report_header, updated_at: new Date().toISOString() }).eq('id', 1);
  ...
  const performGlobalSearch = (query: string): GlobalSearchResult[] => {
    ...
    visibleSavingsAccounts.forEach(s => {
      if (s.account_number.toLowerCase().includes(q) || s.client?.full_name.toLowerCase().includes(q)) {
        results.push({
          type: 'Savings',
          id: s.id,
