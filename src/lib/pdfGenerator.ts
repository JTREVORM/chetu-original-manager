...
function drawVoucherFooter(doc: jsPDF, roles: [string, string, string]) {
  ...
  doc.setTextColor(...V_MUTED);
  doc.text(
    "Chetu Microfinance Ltd — confidential, system generated voucher.",
    pageWidth / 2,
    pageHeight - 9.5,
    { align: "center" },
  );
...
function drawPDFHeader(doc: jsPDF, documentTitle: string, docNumber?: string) {
  ...
  doc.setFontSize(8.5);
  doc.text(
    docNumber ? `Ref: ${docNumber}` : `Date: ${new Date().toISOString().split("T")[0]}`,
    160.5,
    30,
    { align: "center" },
  );
...
export function generateLoanAgreementPDF(loan: Loan) {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        ...
        "Total Payable",
        formatUGX(loan.total_amount_payable),
      ],
      [
        "Weekly Installment",
        formatUGX(loan.weekly_installment),
        "Duration",
        `${loan.loan_period_weeks} Weeks`,
      ],
  ...
  if (loan.schedule && loan.schedule.length > 0) {
    autoTable(doc, {
      startY: nextY + 4,
      head: [
        ["Wk #", "Due Date", "Installment", "Principal", "Interest", "Remaining Bal", "Status"],
      ],
...
export function generateLoanStatementPDF(loan: Loan, repayments: LoanRepayment[]) {
  ...
  doc.text(
    `Principal: ${formatUGX(loan.principal_amount)} | Outstanding: ${formatUGX(loan.outstanding_balance)}`,
    14,
    y + 5,
  );
...
export async function generateBankTransactionPDF(tx: BankTransaction) {
  ...
  drawAmountBox(
    doc,
    `${tx.transaction_type} Amount`,
    tx.amount,
    afterTable,
    isDeposit ? "green" : "red",
  );

  drawVoucherFooter(doc, [
