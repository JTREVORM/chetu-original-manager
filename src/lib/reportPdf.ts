...
export async function exportReportPDF({
  ...
  const now = new Date();
  const generatedOn = now.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' });
  ...
  const drawHeader = () => {
    ...
    doc.text(`${rows.length} record${rows.length === 1 ? '' : 's'}`, pageWidth - 12, 19.5, { align: 'right' });
  ...
  autoTable(doc, {
    ...
    startY,
    styles: { fontSize: 6.9, cellPadding: 1.5, overflow: 'linebreak', textColor: [51, 65, 85], lineColor: [226, 232, 240], lineWidth: 0.1 },
    headStyles: { fillColor: NAVY, textColor: 255, fontStyle: 'bold', fontSize: 6.9, cellPadding: 2 },
  ...
  if (totals?.length) {
    const afterTable = (doc as unknown as { lastAutoTable?: { finalY: number } }).lastAutoTable?.finalY ?? startY;
    let y = afterTable + 6;
    if (y > pageHeight - 24) {
      doc.addPage();
      drawHeader();
