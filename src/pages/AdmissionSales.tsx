...
export const AdmissionSales: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [feesRes, officersRes] = await Promise.all([
        ...
        supabase.from('profiles').select('id, full_name, branch_ids, status').eq('role', 'Loan Officer').order('full_name'),
      ]);
      if (cancelled) return;
  ...
  const enriched = useMemo(() => {
    ...
    return rows.map((r) => {
      ...
      const officerName =
        group?.loan_officer_name || (group?.loan_officer_id ? officerById.get(group.loan_officer_id)?.full_name : '') || '—';
      return {
        ...r,
  ...
  const filtered = useMemo(() => {
    ...
    return enriched.filter((r) => {
      ...
      if (branchLocked && myBranches.length && r.branch_id && !myBranches.includes(r.branch_id)) return false;
      ...
      if (q) {
        const hay = `${r.group_name} ${r.group_code} ${r.member_name} ${r.member_code}`.toLowerCase();
  ...
  return (
    <div className="space-y-4 pb-16">
      <h1 className="text-lg font-bold text-slate-900 sm:text-xl">Admission &amp; Passbook Sale (Member)</h1>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-xs sm:p-4">
        <div className="grid grid-cols-1 items-end gap-x-4 gap-y-3 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <div>
            ...
            <input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="form-field" />
          </div>
      ...
      <div className="hidden rounded-lg border border-slate-200 bg-white shadow-xs md:block">
        <table className="w-full table-fixed text-left text-[11px]">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!loading &&
              results.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50 [&>td]:overflow-hidden [&>td]:text-ellipsis [&>td]:whitespace-nowrap">
                  <td className="px-2 py-2.5 text-slate-600" title={r.branch_name}>{r.branch_name}</td>
                  <td className="px-2 py-2.5 text-slate-600" title={r.officer_name}>{r.officer_name}</td>
                  <td className="px-2 py-2.5 text-slate-600">Umodzi Micro Loan</td>
                  <td className="px-2 py-2.5 text-slate-600" title={r.group_code}>{r.group_code}</td>
                  <td className="px-2 py-2.5 text-slate-600" title={r.group_name}>{r.group_name}</td>
                  <td className="px-2 py-2.5 text-slate-600" title={r.member_code}>{r.member_code}</td>
                  <td className="px-2 py-2.5 font-semibold text-slate-900" title={r.member_name}>{r.member_name}</td>
                  <td className="px-2 py-2.5 text-slate-600">{new Date(r.created_at).toLocaleDateString()}</td>
                  <td className="px-2 py-2.5 text-right text-slate-700">{money(r.admission_fee)}</td>
                  <td className="px-2 py-2.5 text-right text-slate-700">{money(r.passbook_fee)}</td>
                </tr>
              ))}
          </tbody>
      ...
      <div className="space-y-3 md:hidden">
        ...
        {!loading && hasSearched && results.length === 0 && (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center">
            <Receipt className="mx-auto mb-2 h-6 w-6 text-slate-300" />
            <p className="text-sm font-bold text-slate-600">No admission or passbook sales found.</p>
          </div>
        )}
        {!loading &&
          results.map((r) => (
            <div key={r.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-xs">
              <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                <div className="min-w-0">
                  <h3 className="break-words text-sm font-bold leading-snug text-slate-900">{r.member_name}</h3>
              ...
              <dl className="mt-3 space-y-2 border-t border-slate-100 pt-3">
                {[
                  ...
                  <div key={f.label} className="grid grid-cols-[minmax(0,40%)_minmax(0,60%)] items-start gap-3">
                    <dt className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">{f.label}</dt>
                    <dd className="break-words text-right text-xs font-semibold text-slate-800">{f.value}</dd>
                  </div>
                ))}
              </dl>
            </div>
        ...
        {!loading && results.length > 0 && (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-xs font-bold text-slate-800">
            <div className="flex justify-between"><span>Admission total</span><span>{money(totals.admission)}</span></div>
            <div className="mt-1 flex justify-between"><span>Passbook total</span><span>{money(totals.passbook)}</span></div>
            <div className="mt-1 flex justify-between"><span>CRB total</span><span>{money(totals.crb)}</span></div>
            <div className="mt-1 flex justify-between border-t border-slate-200 pt-1"><span>Grand total</span><span>{money(totals.total)}</span></div>
          </div>
        )}
      </div>
