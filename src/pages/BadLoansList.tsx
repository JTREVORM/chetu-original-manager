...
export const BadLoansList: React.FC = () => {
  ...
  const [applied, setApplied] = useState({ branchId: '', officerId: '', groupId: '', search: '', ageing: '' });
  ...
  const rows = useMemo(() => {
    ...
    return loans
      .filter((l) => ['Active', 'Overdue', 'Defaulted'].includes(l.status) && Number(l.outstanding_balance) > 0)
      .map<BadRow | null>((l) => {
        ...
        const since = last?.payment_date || l.first_repayment_date || l.disbursed_at || l.created_at;
  ...
  const runSearch = () => {
    setHasSearched(true);
    setApplied({ branchId: scope.branchId, officerId: scope.officerId, groupId: scope.groupId, search, ageing });
  };
  ...
  const declare = async () => {
    ...
    try {
      ...
      addToast('error', 'Declaration failed', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns: MisColumn<BadRow>[] = [
    ...
    { key: 'out', label: 'Outstanding', width: '9%', align: 'right', render: (r) => money(r.loan.outstanding_balance) },
    {
      key: 'age',
      label: 'Ageing (Days)',
      width: '8%',
    ...
    { key: 'last', label: 'Status', width: '8%', render: (r) => (r.loan.is_bad_debt ? 'Bad Debt' : r.loan.status) },
    {
      key: 'comment',
      label: 'Comment',
      width: '12%',
  ...
  const visibleColumns = canDeclare ? columns : columns.filter((c) => c.key !== 'pick' && c.key !== 'comment');

  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Bad Loans List" columns={visibleColumns} rows={filtered} />}>Bad Loans List</MisPageTitle>
