...
const StatusPill: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span
      className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-bold ${tone[status] || "bg-slate-200 text-slate-700"}`}
    >
      {status.replace(/_/g, " ")}
    </span>
  );
...
export const BusinessDayControl: React.FC = () => {
  ...
  const todaysDays = useMemo(
    () => businessDays.filter((d) => d.business_date === today),
    [businessDays, today],
  );
  const todaysOfficerDays = useMemo(
    () => officerDays.filter((d) => d.business_date === today),
    [officerDays, today],
  );
  const myDay = useMemo(
  ...
  const awaitingApproval = todaysOfficerDays.filter(
    (d) => d.status === "SUBMITTED" || d.status === "PENDING_APPROVAL",
  );
  ...
  const mySummary = useMemo(() => {
    ...
    const todaysRepayments = repayments.filter(
      (r) => onToday(r.payment_date) || onToday(r.created_at),
    );
    ...
    return {
      ...
      deposits: todaysSavings
        .filter((t) => t.transaction_type !== "Withdrawal")
        .reduce((s, t) => s + Number(t.amount || 0), 0),
      withdrawals: todaysSavings
        .filter((t) => t.transaction_type === "Withdrawal")
        .reduce((s, t) => s + Number(t.amount || 0), 0),
    };
  ...
  const pending = useMemo(() => {
    ...
    if (unapprovedClients.length)
      items.push(`${unapprovedClients.length} member admission(s) still awaiting approval`);
  ...
  const reviewData = useMemo(() => {
    ...
    return {
      clients: clients.filter(
        (c) => on(c.date_registered) && c.loan_officer_id === reviewing.officer_id,
      ),
    ...
    run(
      ...
      async () => {
        ...
        await logAudit(
          "Business Day Closed",
          "Business Day",
          `Closed ${today} for ${branchName(branchId)}.`,
          branchId,
        );
        await sendNotification({
    ...
    run(
      ...
      async () => {
        ...
        await logAudit(
          "Working Day Started",
          "Business Day",
          `${user?.full_name} started work on ${today}.`,
        );
    ...
    run(
      ...
      async () => {
        ...
        await logAudit(
          ...
          "Business Day",
          `${user?.full_name} closed their day for ${today}.`,
          myDay.id,
        );
        await sendNotification({
        ...
        addToast(
          "success",
          "Day submitted",
          "Your day is now with your Branch Manager for approval.",
        );
      },
    ...
    run(
      ...
      async () => {
        ...
        await logAudit(
          ...
          "Business Day",
          `Approved ${d.business_date} for officer ${d.officer_id}.`,
          d.id,
        );
        await sendNotification({
    ...
    run(
      ...
      async () => {
        ...
        if (!rejectReason.trim())
          throw new Error("Give a reason so the officer knows what to correct.");
        const { error } = await supabase
          .from("officer_days")
          .update({
            status: "REJECTED",
        ...
        await logAudit(
          ...
          "Business Day",
          `Sent back ${rejectFor.business_date}: ${rejectReason}`,
          rejectFor.id,
        );
        await sendNotification({
    ...
    run(
      ...
      async () => {
        ...
        addToast(
          ...
          approve ? "Access granted" : "Request rejected",
          "The officer has been notified.",
        );
      },
    ...
    <div className="space-y-5 pb-12">
      <div className="page-banner p-5 sm:p-6">
        ...
        <p className="mt-1 max-w-2xl text-[13px] leading-relaxed text-blue-100">
          Work is only permitted inside a business day a manager has opened. All times below are the
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-5 gap-y-1 text-[12px] text-slate-600">
                <span>
                  Date: <strong className="text-slate-900">{shortDate(today)}</strong>
                </span>
                <span>
                  Server time:{" "}
                ...
                <span>
                  Business day: <StatusPill status={businessDayStatus} />
                </span>
                {isOfficer && (
                  <span>
                    My day: <StatusPill status={officerDayStatus} />
                  </span>
                )}
          ...
          {isOfficer && (
            <div className="flex flex-wrap gap-2">
              ...
              {(officerDayStatus === "ACTIVE" ||
                officerDayStatus === "REJECTED" ||
                officerDayStatus === "SPECIAL_ACCESS") && (
                <button
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          <section className="rounded-lg border border-slate-200 bg-white shadow-xs lg:col-span-2">
            <header className="border-b border-slate-200 px-5 py-3.5">
              ...
              <p className="mt-0.5 text-[12px] text-slate-500">
                These totals are submitted with your day for approval.
              </p>
            </header>
          ...
          <section className="rounded-lg border border-slate-200 bg-white shadow-xs">
            <header className="border-b border-slate-200 px-5 py-3.5">
              ...
              <p className="mt-0.5 text-[12px] text-slate-500">
                For weekends or before the day is opened.
              </p>
            </header>
      ...
      {isManagement && !isAuditor && (
        <>
          ...
          <BusinessDayStatusCard
            ...
            closedByName={
              staffNames[(selectedDay as { closed_by?: string } | undefined)?.closed_by || ""] ||
              "—"
            }
            stats={selectedStats}
            now={now}
          ...
          <section className="rounded-lg border border-slate-200 bg-white shadow-xs">
            <header className="border-b border-slate-200 px-5 py-3.5">
              ...
              <p className="mt-0.5 text-[12px] text-slate-500">
                {branchName(selectedBranchId)} · {shortDate(today)}
              </p>
            </header>
                  ...
                  {todaysDays.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-4 py-8 text-center text-slate-400">
                        No branch has been opened for {shortDate(today)}.
                      </td>
                    </tr>
                  )}
                  {todaysDays.map((d) => (
                    <tr key={d.id} className="[&>td]:whitespace-nowrap [&>td]:px-4 [&>td]:py-2.5">
                      ...
                      <td>
                        {
                          todaysOfficerDays.filter(
                            (o) => o.business_day_id === d.id && o.status === "ACTIVE",
                          ).length
                        }
                      </td>
                      <td className="text-right">
                        {d.status === "OPEN" && (
                          <button
                            onClick={() => closeDay(d.branch_id, d.id)}
                            disabled={!!busy}
                  ...
                  {todaysOfficerDays.length === 0 && (
                    <tr>
                      <td colSpan={7} className="px-4 py-8 text-center text-slate-400">
                        No officer has started work today.
                      </td>
                    </tr>
                  )}
                  {todaysOfficerDays.map((d) => (
                    <tr key={d.id} className="[&>td]:whitespace-nowrap [&>td]:px-4 [&>td]:py-2.5">
                      ...
                      <td className="text-right font-bold">
                        {d.summary?.collected != null
                          ? formatUGX(Number(d.summary.collected))
                          : "—"}
                      </td>
                      <td className="text-right">
            ...
            <header className="border-b border-slate-200 px-5 py-3.5">
              <h2 className="text-sm font-bold text-slate-900">
                ...
                {pendingRequests.length > 0 && (
                  <span className="ml-1 text-amber-600">({pendingRequests.length} pending)</span>
                )}
              </h2>
            </header>
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {accessRequests.slice(0, 20).map((r) => (
                    <tr key={r.id} className="[&>td]:px-4 [&>td]:py-2.5">
                      ...
                      <td className="max-w-72 truncate" title={r.reason}>
                        {r.reason}
                      </td>
                      <td>
                        <StatusPill status={r.status} />
                      ...
                      <td className="whitespace-nowrap">
                        {r.expires_at ? shortTime(r.expires_at) : "—"}
                      </td>
                      <td className="whitespace-nowrap text-right">
                        {r.status === "Pending" && (
                          <span className="inline-flex gap-1.5">
                            <button
                              onClick={() => decideRequest(r.id, r.requester_id, true)}
                              disabled={!!busy}
                            ...
                            <button
                              onClick={() => decideRequest(r.id, r.requester_id, false)}
                              disabled={!!busy}
                              className="rounded border border-red-300 px-2.5 py-1 text-[11px] font-bold text-red-700 hover:bg-red-50"
                            >
                              Reject
                            </button>
                          </span>
      ...
      {(isManagement || isAuditor) && (
        <section className="rounded-lg border border-slate-200 bg-white shadow-xs">
          <header className="border-b border-slate-200 px-5 py-3.5">
            ...
            <p className="mt-0.5 text-[12px] text-slate-500">
              Append-only. Entries cannot be edited or removed by anyone.
            </p>
          </header>
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {audit.slice(0, 60).map((a) => (
                  <tr key={a.id} className="[&>td]:px-4 [&>td]:py-2">
                    <td className="whitespace-nowrap">
                      {shortDate(a.created_at)} {shortTime(a.created_at)}
                    </td>
          ...
          <div className="w-full max-w-lg rounded-lg bg-white shadow-2xl">
            ...
            <div className="space-y-4 p-5">
              ...
              <p className="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-[12px] text-slate-700">
                I confirm that my day's activities are complete and I am submitting this day for
                approval.
              </p>
              ...
              <div className="flex justify-end gap-2">
                <button
                  ...
                  className="rounded-lg bg-slate-100 px-4 py-2 text-[13px] font-semibold text-slate-700"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/50 p-4">
          <div className="w-full max-w-4xl rounded-lg bg-white shadow-2xl">
            ...
            <div className="max-h-[70vh] space-y-4 overflow-y-auto p-5">
                ...
                {Object.entries(reviewing.summary || {}).map(([k, v]) => (
                  <div key={k} className="bg-white px-3 py-2">
                    ...
                    <dd className="mt-0.5 text-[13px] font-bold text-slate-900">
                      {["collected", "deposits", "withdrawals"].includes(k)
                        ? formatUGX(Number(v))
                        : String(v)}
                    </dd>
                  </div>
              ...
              <ReviewList
                title={`Members registered (${reviewData.clients.length})`}
                rows={reviewData.clients.map((c) => [c.client_number, c.full_name, c.status])}
              />
              <ReviewList
                title={`Loans touched (${reviewData.loans.length})`}
                rows={reviewData.loans.map((l) => [
                  l.loan_number,
              ...
              />
              <ReviewList
                title={`Repayments (${reviewData.repayments.length})`}
                rows={reviewData.repayments.map((r) => [
                  r.receipt_number,
              ...
              />
              <ReviewList
                title={`Savings movements (${reviewData.savings.length})`}
                rows={reviewData.savings.map((t) => [
              ...
              />
              <ReviewList
                title={`Audit entries (${reviewData.audit.length})`}
                rows={reviewData.audit.map((a) => [
      ...
      {rejectFor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4">
          <div className="w-full max-w-md rounded-lg bg-white shadow-2xl">
            <div className="border-b border-slate-200 px-5 py-3.5">
              <h3 className="text-sm font-bold text-slate-900">
                Send the day back for corrections
              </h3>
            </div>
            <div className="space-y-3 p-5">
              <textarea
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className="rounded-lg bg-chetu-red px-4 py-2 text-[13px] font-bold text-white disabled:opacity-50"
                >
                  Send back
                </button>
              </div>
