...
export const Clients: React.FC = () => {
  ...
  const { clients, clientGroups, branches, updateClient, loans, repayments, savingsAccounts } = useDatabase();
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    setSelectedClient(prev => prev ? { ...prev, ...editFormData, passport_photo: editPhotoPreview || prev.passport_photo } : null);
  ...
  const clientRepayments = selectedClient ? repayments.filter(r => r.client_id === selectedClient.id) : [];
  const clientSavings = selectedClient ? savingsAccounts.find(s => s.client_id === selectedClient.id) : null;
    ...
    <div className="space-y-5 pb-12 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {[
          ...
          { label: 'Active', value: String(filteredClients.filter(c => c.status === 'Active').length), icon: UserCheck },
          {
            label: 'Savings Held',
            value: formatUGX(
              savingsAccounts
...
<TableScroll>
              ...
              {filteredClients.length === 0 && (
                <tr>
                  <td colSpan={8} className="p-10 text-center text-slate-400">
                    ...
                    <Link to="/member-admission" className="font-bold text-[#0B4394]">Members &rsaquo; Member Admission</Link>{' '}
                    to admit one.
                  </td>
                </tr>
              )}
                ...
                <tr key={client.id} className="transition-colors hover:bg-slate-50">
                  <td className="p-3.5">
                    <div className="flex items-center gap-3">
                      <Avatar src={client.passport_photo} name={client.full_name} className="h-9 w-9 shrink-0 rounded-xl ring-2 ring-slate-100" />
                      <div className="min-w-0">
                        <p className="truncate font-bold text-slate-900">{client.full_name}</p>
                        <p className="text-[11px] font-bold text-[#0B4394]">{client.client_number}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="p-3.5 text-slate-700">{client.village}, {client.district}</td>
                  <td className="p-3.5 text-slate-700">{client.date_registered}</td>
                  <td className="p-3.5">
                    <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-bold ${statusPill(client.status)}`}>
                      {client.status}
                    </span>
                  </td>
                  <td className="p-3.5">
        ...
        <div className="divide-y divide-slate-100 md:hidden">
          ...
          {filteredClients.map(client => (
            ...
            >
              <Avatar src={client.passport_photo} name={client.full_name} className="h-10 w-10 shrink-0 rounded-xl ring-2 ring-slate-100" />
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-bold text-slate-900">{client.full_name}</p>
                <p className="text-[11px] text-slate-500">{client.client_number} • {client.phone_number}</p>
              </div>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${statusPill(client.status)}`}>
                {client.status}
              </span>
            ...
            <div className="p-5 brand-gradient text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-200">{selectedClient.client_number} • NIN: {selectedClient.nin}</p>
                </div>
              </div>
            ...
            <div className="flex border-b border-slate-200 bg-slate-50 px-5">
              <button
                ...
                className={`py-3 px-4 text-xs font-bold border-b-2 transition-all ${
                  activeTab === 'info' ? 'border-[#0B4394] text-[#0B4394]' : 'border-transparent text-slate-500'
                }`}
              >
                Personal Info
              </button>
              <button
                ...
                className={`py-3 px-4 text-xs font-bold border-b-2 transition-all ${
                  activeTab === 'loans' ? 'border-[#0B4394] text-[#0B4394]' : 'border-transparent text-slate-500'
                }`}
              >
              ...
              <button
                ...
                className={`py-3 px-4 text-xs font-bold border-b-2 transition-all ${
                  activeTab === 'savings' ? 'border-[#0B4394] text-[#0B4394]' : 'border-transparent text-slate-500'
                }`}
              >
                Savings Vault
              </button>
              <button
                ...
                className={`py-3 px-4 text-xs font-bold border-b-2 transition-all ${
                  activeTab === 'repayments' ? 'border-[#0B4394] text-[#0B4394]' : 'border-transparent text-slate-500'
                }`}
              >
                ...
                <div className="space-y-4 text-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div>
                      ...
                      <p className="font-semibold text-slate-900">{selectedClient.gender} ({selectedClient.date_of_birth})</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Occupation / Employer</p>
                      <p className="font-semibold text-slate-900">{selectedClient.occupation} ({selectedClient.employer || 'Self'})</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Alternative Phone</p>
                      <p className="font-semibold text-slate-900">{selectedClient.alt_phone_number || 'N/A'}</p>
                    </div>
                    <div className="sm:col-span-2">
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Email Address</p>
                      <p className="font-semibold text-slate-900">{selectedClient.email || 'N/A'}</p>
                    </div>
                    <div className="sm:col-span-2">
                      <p className="text-[10px] font-bold text-slate-400 uppercase">Client Group</p>
                      <p className="font-semibold text-[#0B4394]">
                        {clientGroups.find(g => g.id === selectedClient.group_id)?.group_name || 'Unassigned'}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-2">
                    ...
                    <p><strong>Physical Address:</strong> {selectedClient.physical_address}</p>
                    <p><strong>Village / LC1:</strong> {selectedClient.village}</p>
                    <p><strong>Parish / Sub County:</strong> {selectedClient.parish}, {selectedClient.sub_county}</p>
                    <p><strong>District:</strong> {selectedClient.district}</p>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'loans' && (
                <div className="space-y-3">
                  {clientLoans.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-8">No loan records for this client.</p>
                  ) : (
                    clientLoans.map(loan => (
                      <div key={loan.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-slate-900 text-xs">{loan.loan_number}</span>
                          <span className="px-2 py-0.5 bg-blue-100 text-blue-800 text-[10px] font-bold rounded">
                            {loan.status}
                          </span>
                        </div>
                        <p className="text-xs text-slate-600 mt-1">Principal: {formatUGX(loan.principal_amount)}</p>
                        <p className="text-xs text-slate-600">Outstanding Balance: {formatUGX(loan.outstanding_balance)}</p>
                      </div>
                    ))
                  )}
                </div>
                  ...
                  {clientSavings ? (
                    <div>
                      <div className="flex items-center justify-between border-b pb-2">
                        ...
                        <span className="font-black text-[#0B4394]">{clientSavings.account_number}</span>
                      </div>
                      <div className="pt-3">
                        <p className="text-[10px] font-bold text-slate-400 uppercase">Vault Balance</p>
                        <h3 className="text-2xl font-black text-slate-900 mt-1">{formatUGX(clientSavings.balance)}</h3>
                        <p className="text-slate-500 mt-1">Status: Active Individual Savings Account</p>
                      </div>
                    </div>
                  ) : (
              ...
              {activeTab === 'repayments' && (
                <div className="space-y-3">
                  {clientRepayments.length === 0 ? (
                    <p className="text-xs text-slate-400 text-center py-8">No repayment transactions logged.</p>
                  ) : (
                    clientRepayments.map(rep => (
                      <div key={rep.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-center justify-between text-xs">
                        <div>
                          <p className="font-bold text-slate-900">{rep.receipt_number}</p>
                          <p className="text-slate-500">{rep.payment_date} • {rep.payment_method}</p>
                        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditFormData({ ...editFormData, alt_phone_number: e.target.value })}
                    className="w-full px-3 py-2 border rounded-xl text-xs font-medium"
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  >
                    ...
                    {clientGroups.filter(g => g.status === 'Active').map(g => (
