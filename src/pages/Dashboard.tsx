...
export const Dashboard: React.FC = () => {
  ...
  const m = useMemo(() => {
    ...
    return {
      ...
      disbursedThisMonth,
      activeMembers: clients.filter(
        (c) => c.status === "Active" && c.approval_status === "Approved",
      ).length,
      activeGroups: clientGroups.filter(
  ...
  const disbursementTrend = months.map(({ key, label }) => ({
    month: label,
    Disbursed: loans
      .filter((l) => (l.disbursed_at || "").startsWith(key))
      .reduce((s, l) => s + Number(l.principal_amount || 0), 0),
    Collected: repayments
      .filter((r) => r.payment_date.startsWith(key))
  ...
  const myBranchName =
    branches.length === 1 ? branches[0]!.branch_name : `${branches.length} branches`;
  const scopeLabel = isLoanOfficer
    ? "My portfolio"
    : isBranchManager
      ? myBranchName
      : "All branches";
  ...
  const queue = useMemo(() => {
    ...
    if (isLoanOfficer) {
      items.push({
        label: "My rejected applications",
        count: m.rejectedApps,
        to: "/loan-rejected",
      });
      items.push({
        label: "Members awaiting approval",
        count: m.pendingMembers,
        to: "/member-waiting-approval",
      });
      items.push({
        label: "Groups awaiting approval",
        count: m.pendingGroups,
        to: "/groups/waiting-approval",
      });
      items.push({
        label: "Loans to disburse",
        count: m.awaitingDisburse,
        to: "/loan-waiting-disburse",
      });
    } else {
      items.push({
  ...
  const outstandingLabel = isLoanOfficer
    ? "My outstanding portfolio"
    : isBranchManager
      ? "Branch portfolio"
  ...
  return (
    <div className="space-y-4 pb-16">
      ...
      <div className="overflow-hidden rounded-lg bg-[#0B4394] text-white shadow-xs">
        <div className="flex flex-wrap items-start justify-between gap-4 px-5 py-5">
          <div className="min-w-0">
            ...
            <p className="mt-1.5 max-w-xl text-[13px] leading-relaxed text-blue-100">
              {welcomeLine}
            </p>
          </div>
      ...
      <div
        className={`grid grid-cols-1 gap-3 sm:grid-cols-2 ${institutionWide ? "xl:grid-cols-4" : "xl:grid-cols-3"}`}
      >
        <Kpi
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Mini
          ...
          value={`${m.collectionRate.toFixed(1)}%`}
          hint="of instalments due to date"
        />
        <Mini
          label="Arrears"
          value={formatUGX(m.arrears)}
          hint={`${m.inArrearsCount} loan(s) behind`}
          tone={m.arrears > 0 ? "negative" : undefined}
        />
        <Mini
          label="Disbursed this month"
          value={formatUGX(m.disbursedThisMonth)}
                ...
                <li key={q.label}>
                  ...
                  >
                    ...
                    <span className="flex shrink-0 items-center gap-2">
                      <span className="rounded bg-amber-100 px-2 py-0.5 text-[12px] font-bold text-amber-800">
                        {q.count}
                      </span>
            ...
            <div className="h-56 px-2 pb-2 pt-3">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip
                    formatter={(v: number, n: string) => [formatUGX(Number(v)), n]}
                    contentStyle={{ fontSize: 12, borderRadius: 6 }}
                  />
                </PieChart>
              </ResponsiveContainer>
                ...
                <tbody className="divide-y divide-slate-100">
                  {breakdown.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50">
                      ...
                      <td
                        className={`px-4 py-2.5 text-right tabular-nums font-semibold ${r.arrears > 0 ? "text-chetu-red" : "text-slate-400"}`}
                      >
...
const Kpi: React.FC<{ label: string; value: string; foot?: string; tone?: string }> = ({
  label,
  value,
  foot,
  tone,
  ...
  <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-xs">
    ...
    <p className={`mt-1.5 text-2xl font-bold tabular-nums ${tone ? TONE[tone] : "text-slate-900"}`}>
      {value}
    </p>
    {foot && (
...
const Mini: React.FC<{ label: string; value: string; hint?: string; tone?: string }> = ({
  label,
  value,
  hint,
  tone,
  ...
  <div className="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-xs">
    ...
    <p className={`mt-1 text-lg font-bold tabular-nums ${tone ? TONE[tone] : "text-slate-900"}`}>
      {value}
    </p>
...
const MyGroups: React.FC<{
  ...
  if (rows.length === 0) {
    return (
      <p className="px-4 py-10 text-center text-[13px] text-slate-400">
        No approved groups assigned to you yet.
      </p>
    );
  }
