  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.loan_officer_id) {
      addToast(
        ...
        "Select the loan officer who will manage this group.",
      );
      return;
    }
    setSaving(true);
    try {
      ...
      if (created.approval_status === "Pending") {
        addToast(
          "success",
          "Sent for Approval",
          `${created.group_name} (${created.group_code}) was submitted to your Branch Manager for approval.`,
        );
        ...
        addToast(
          "success",
          "Group Created",
          `Created group ${created.group_name} (${created.group_code})`,
        );
      ...
      addToast(
        "error",
        "Creation Failed",
        err?.message || "Could not create the group. Please try again.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 pb-16">
      ...
      <form
        ...
        className="rounded-lg border border-slate-200 bg-white p-4 shadow-xs sm:p-5"
      >
        <Section
          title="Group Identity"
          description="Basic details identifying the group in the register."
          icon={Building}
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              {lockedBranch ? (
                <input
                  readOnly
                  value={lockedBranch.branch_name}
                ...
                >
                  <option value="">
                    {activeBranches.length ? "Select Branch" : "No branches available"}
                  </option>
        ...
        <Section
          title="Assignment"
          description="Officer responsible for this group."
          icon={UserPlus}
        >
          <div>
            ...
            {isLoanOfficer ? (
              <>
                <input
                  readOnly
                  value={formData.loan_officer_name || user?.full_name || ""}
                  className={`${field} bg-slate-100 font-bold`}
                />
                <p className="mt-1 text-[10px] font-medium text-slate-500">
                  Locked to your account — you will manage this group.
                </p>
              </>
            ) : (
              <>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({
                      ...formData,
                      loan_officer_id: e.target.value,
                ...
                <p className="mt-1 text-[10px] font-medium text-slate-500">
                  Only officers attached to the selected branch are listed.
                </p>
              </>
            )}
          </div>
        ...
        <Section
          ...
          description="Where and when the group convenes each week."
          icon={MapPin}
        >
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div>
      ...
      <p className="flex items-center gap-1.5 text-[11px] font-medium text-slate-400">
        <Calendar className="h-3.5 w-3.5" />A unique group code is generated automatically on save.
      </p>
    </div>
  );
};
