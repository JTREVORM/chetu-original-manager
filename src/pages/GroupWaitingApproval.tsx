...
export const GroupWaitingApproval: React.FC = () => {
  ...
  const { clientGroups, updateClientGroup, approveClientGroup, rejectClientGroup, branches } =
    useDatabase();
              ...
              <tbody className="divide-y divide-slate-100 text-[11px]">
                ...
                {pendingGroups.map((group) => (
                  <tr key={group.id} className="transition-colors hover:bg-slate-50">
                    <td className="truncate px-2 py-3 font-semibold text-slate-700">
                      {group.branch || "—"}
                    </td>
                    ...
                    <td className="truncate px-2 py-3 font-bold text-slate-900">
                      {group.group_name}
                    </td>
                    <td className="truncate px-2 py-3 text-slate-700">
                      {group.meeting_day || "—"}
          ...
          {pendingGroups.map((group) => (
            <div key={group.id} className="space-y-2 p-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-600">
                ...
                <p>
                  LO: <strong className="text-slate-900">{group.loan_officer_name || "—"}</strong>
                </p>
                <p>
                  Day: <strong className="text-slate-900">{group.meeting_day || "—"}</strong>
                </p>
                <p>
                  Formed: <strong className="text-slate-900">{group.formation_date || "—"}</strong>
                </p>
              </div>
          ...
          <div className="w-[calc(100vw-1.5rem)] max-w-md overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl md:w-full">
            ...
            <form onSubmit={handleRejectSubmit} className="space-y-4 p-6 text-xs">
              <p className="text-slate-600">
                Rejecting <strong className="text-slate-900">{rejectingGroup.group_name}</strong> (
                {rejectingGroup.group_code}). The Loan Officer will be able to edit and resubmit it.
              </p>
              <div>
              ...
              <div className="flex flex-col-reverse gap-2 border-t pt-3 md:flex-row md:justify-end">
                ...
                <button
                  type="submit"
                  disabled={busyId === rejectingGroup.id}
