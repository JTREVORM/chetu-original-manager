...
export const LoanRejectedList: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q) {
        const hay =
          `${r.group_name} ${r.client.full_name} ${r.client.client_number} ${r.app.application_number}`.toLowerCase();
  ...
  const confirmResubmit = async () => {
    ...
    try {
      ...
      addToast(
        "success",
        "Sent for approval",
        `${confirming.app.application_number} is back in the approval queue.`,
      );
      setConfirming(null);
    } catch (error) {
      addToast(
  ...
  const columns: MisColumn<RejectedRow>[] = [
    ...
    },
    {
      ...
      label: "Rejected On",
      width: "8%",
      render: (r) => shortDate(r.app.updated_at || r.app.created_at),
    },
    {
      key: "reason",
      label: "Reason",
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle
        right={<ReportExportButtons title="Loan Rejected List" columns={columns} rows={filtered} />}
      >
        Loan Rejected List
      </MisPageTitle>
        ...
        {confirming && (
          <div className="space-y-4 text-xs">
            <p className="text-slate-600">
              Send <strong className="text-slate-900">{confirming.app.application_number}</strong>{" "}
              for <strong className="text-slate-900">{confirming.client.full_name}</strong> back to
              the Branch Manager for approval?
            </p>
            <div className="rounded border border-amber-200 bg-amber-50 p-3">
              <p className="font-bold uppercase text-[10px] tracking-wide text-amber-800">
                Rejection reason
              </p>
              <p className="mt-1 text-slate-700">
                {confirming.app.rejection_reason || "No reason recorded."}
              </p>
            </div>
            <p className="text-slate-500">
              Correct the member or loan details first if the reason still applies — resubmitting
