...
export const LoanSecurityReturnList: React.FC = () => {
  ...
  const processReturn = async (row: ReturnRow, amount: number) => {
    ...
    try {
      ...
      addToast('success', 'Security Returned', `${money(amount)} returned to ${row.client.full_name}.`);
      setDetailsFor(null);
      await refetch({ silent: true });
    } catch (error) {
      addToast('error', 'Return failed', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns: MisColumn<ReturnRow>[] = [
    ...
    { key: 'dur', label: 'Duration', width: '7%', align: 'center', render: (r) => `${r.loan.loan_period_weeks || 0} wks` },
    { key: 'prev', label: 'Previous Amt', width: '10%', align: 'right', render: (r) => money(r.previous_amount) },
    { key: 'pres', label: 'Present Amt', width: '10%', align: 'right', render: (r) => money(r.present_amount) },
    {
      key: 'action',
      label: 'Action',
      width: '7%',
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Loan Security Return List" columns={columns} rows={filtered} />}>Loan Security Return List</MisPageTitle>
      ...
      >
        ...
        <Field label="Return Date">
          <input type="date" value={returnDate} onChange={(e) => setReturnDate(e.target.value)} className="form-field" />
        </Field>
      ...
      <SecurityReturnModal row={detailsFor} readOnly={!canReturn} saving={saving} returnDate={returnDate} onClose={() => setDetailsFor(null)} onSubmit={processReturn} />
    </div>
  );
};
    ...
    <MisModal open onClose={onClose} title="Loan Security Return Details" width="max-w-2xl">
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
        ...
        <Field label="Disburse Date">
          <input readOnly value={shortDate(row.loan.disbursed_at)} className="form-field bg-slate-100" />
        </Field>
        <Field label="Principal">
          <input readOnly value={money(row.loan.principal_amount)} className="form-field bg-slate-100" />
        </Field>
        <Field label="Interest">
          <input readOnly value={money(row.loan.total_interest_amount)} className="form-field bg-slate-100" />
        </Field>
