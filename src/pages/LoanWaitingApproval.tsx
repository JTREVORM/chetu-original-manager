...
export const LoanWaitingApproval: React.FC = () => {
  ...
  const { loanApplications, clients, clientGroups, loanProducts, approveLoanApplication, rejectLoanApplication } = useDatabase();
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q && !`${r.group_name} ${r.client.full_name} ${r.client.client_number} ${r.app.application_number}`.toLowerCase().includes(q)) return false;
      return true;
    });
  ...
  const preview = useMemo(() => {
    ...
    return {
      ...
      errors: validateLoanRequest(reviewing.app.requested_amount, reviewing.app.requested_weeks, product),
    };
  ...
  const approve = async () => {
    ...
    try {
      ...
      addToast('error', 'Approval failed', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const reject = async () => {
    if (!rejecting) return;
    if (!reason.trim()) return addToast('warning', 'Reason required', 'Tell the officer what to fix.');
    setBusy(true);
    try {
      await rejectLoanApplication(rejecting.app.id, reason.trim());
      addToast('info', 'Application rejected', `${rejecting.app.application_number} moved to the rejected list.`);
      setRejecting(null);
      setReason('');
      ...
      render: (r) =>
        canReview ? (
          <div className="flex items-center justify-center gap-1">
            ...
            <ActionButton onClick={() => { setRejecting(r); setReason(''); }} title="Reject this application" tone="red">
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Waiting for Approval" columns={columns} rows={filtered} />}>
        Waiting for Approval
      </MisPageTitle>

      {!canReview && (
        <p className="rounded border border-slate-200 bg-slate-50 p-2.5 text-[11px] text-slate-500">
          Approving a loan is a Branch Manager or Administrator action. These are the applications you submitted that
          are still waiting.
        </p>
      )}
      ...
      {/* Review + approve */}
      <MisModal open={!!reviewing} onClose={() => setReviewing(null)} title="Review Loan Application" width="max-w-2xl">
        {reviewing && (
          <div className="space-y-4 text-xs">
            <div className="rounded border border-slate-200 bg-slate-50 p-3">
              <p className="text-sm font-bold text-slate-900">{reviewing.client.full_name}</p>
              <p className="text-slate-500">
                {reviewing.app.application_number} • {reviewing.group_name} • {reviewing.branch_name}
              </p>
            ...
            {!preview ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 rounded border border-slate-200 p-3 sm:grid-cols-3">
                  ...
                  <Figure label="Total payable" value={formatUGX(preview.calc.totalAmountPayable)} />
                  <Figure label="Weekly instalment" value={formatUGX(preview.calc.weeklyInstallment)} />
                  <Figure label="First repayment" value={shortDate(preview.calc.firstRepaymentDate)} />
                  <Figure label="Final due" value={shortDate(preview.calc.finalDueDate)} />
                </div>

                <div className="rounded border border-amber-200 bg-amber-50 p-3">
                  <p className="mb-1.5 text-[10px] font-bold uppercase tracking-wide text-amber-800">Charges &amp; deductions</p>
                  <dl className="space-y-1">
                    {preview.fees.map((f) => (
                      <div key={f.label} className={`flex justify-between ${f.emphasis ? 'border-t border-amber-200 pt-1 font-bold' : ''}`}>
            ...
            <div className="flex flex-col-reverse gap-2 border-t border-slate-200 pt-3 sm:flex-row sm:justify-between">
              ...
              <div className="flex flex-col-reverse gap-2 sm:flex-row">
                <button type="button" onClick={() => setReviewing(null)} className="rounded bg-slate-100 px-4 py-2 font-bold text-slate-700">
                  Close
                </button>
                {canReview && (
                  <button
      ...
      {/* Reject */}
      <MisModal open={!!rejecting} onClose={() => setRejecting(null)} title="Reject Loan Application" width="max-w-md">
        {rejecting && (
          <div className="space-y-4 text-xs">
            <p className="text-slate-600">
              Rejecting <strong className="text-slate-900">{rejecting.app.application_number}</strong> for{' '}
              <strong className="text-slate-900">{rejecting.client.full_name}</strong>. It moves to the Loan Rejected
              List, where the officer can correct and resubmit it.
            </p>
            <div>
              <label className="form-label" htmlFor="why">Reason for rejection *</label>
              <textarea
                id="why"
                rows={3}
                value={reason}
