...
export const LoanWaitingDisburse: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q && !`${r.group_name} ${r.client.full_name} ${r.client.client_number} ${r.loan.loan_number}`.toLowerCase().includes(q)) return false;
      return true;
    });
  ...
  const confirm = async () => {
    ...
    try {
      ...
      addToast('success', 'Loan disbursed', `${formatUGX(active.net)} released on ${active.loan.loan_number}.`);
      setActive(null);
    } catch (error) {
      addToast('error', 'Disbursement failed', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const columns: MisColumn<Row>[] = [
    ...
    { key: 'due', label: 'First Repayment', width: '9%', render: (r) => shortDate(r.loan.first_repayment_date) },
    {
      key: 'act',
      label: 'Action',
      width: '6%',
  ...
  const matches =
    active && taken
      ? feesMatchStored(active.loan.principal_amount, taken.processingFee, taken.securityDeposit, taken.crbFee, taken.groupMaintenanceFee)
      : true;

  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Waiting for Disburse" columns={columns} rows={filtered} />}>
        Waiting for Disburse
      </MisPageTitle>

      {!canDisburse && (
        <p className="rounded border border-slate-200 bg-slate-50 p-2.5 text-[11px] text-slate-500">
          Disbursement is recorded by the Loan Officer who will hand over the cash, or by an Administrator. A Branch
          Manager approves the loan but does not release it.
        </p>
      )}
      ...
      <MisModal open={!!active} onClose={() => setActive(null)} title="Disburse Loan" width="max-w-lg">
        {active && taken && std && (
          <div className="space-y-4 text-xs">
            ...
            <div className="space-y-1.5 rounded border border-emerald-200 bg-emerald-50 p-3">
              <p className="text-[10px] font-bold uppercase tracking-wide text-emerald-800">Charges &amp; deductions</p>
              <Line label="Approved principal" value={formatUGX(active.loan.principal_amount)} />
              <Line label={`Processing fee (${FEES.processingFeePct}%)`} value={formatUGX(taken.processingFee)} />
              <Line label={`CRB fee (${FEES.crbFeePct}%)`} value={formatUGX(taken.crbFee)} />
              <Line label={`Security deposit (${FEES.securityDepositPct}%)`} value={formatUGX(taken.securityDeposit)} />
            ...
            {!matches && (
              <p className="rounded border border-amber-200 bg-amber-50 p-2.5 font-semibold text-amber-800">
                The charges stored on this loan differ from the current schedule (which would give processing{' '}
                {formatUGX(std.processingFee)}, CRB {formatUGX(std.crbFee)}, security {formatUGX(std.securityDeposit)}).
                That is expected if the schedule changed after approval — check before releasing cash.
              </p>
            )}

            <div className="flex flex-col-reverse gap-2 border-t border-slate-200 pt-3 sm:flex-row sm:justify-between">
              <div className="flex flex-col gap-2 sm:flex-row">
                <button type="button" onClick={() => generateLoanAgreementPDF(active.loan)}
                ...
                <button type="button" onClick={() => generateDisbursementVoucherPDF(active.loan)}
                  className="inline-flex items-center justify-center gap-2 rounded bg-slate-100 px-3 py-2 font-bold text-slate-700">
...
const Line: React.FC<{ label: string; value: string; bold?: boolean }> = ({ label, value, bold }) => (
  <div className={`flex justify-between ${bold ? 'border-t border-emerald-200 pt-1 font-bold' : ''}`}>
