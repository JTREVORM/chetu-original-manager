...
export const LoanWriteoff: React.FC = () => {
  ...
  const [applied, setApplied] = useState({ branchId: '', officerId: '', groupId: '', search: '', view: 'pending' });
  ...
  const runSearch = () => {
    setHasSearched(true);
    setApplied({ branchId: scope.branchId, officerId: scope.officerId, groupId: scope.groupId, search, view });
  };
  ...
  const confirmWriteoff = async () => {
    ...
    try {
      ...
      addToast('error', 'Write-off failed', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const columns: MisColumn<WriteoffRow>[] = [
    ...
    showingWrittenOff
      ? {
          ...
          width: '9%',
          align: 'right',
          render: (r) => <span className="font-bold text-chetu-red">{money(r.loan.writeoff_amount ?? 0)}</span>,
        }
      : {
          key: 'out',
          label: 'Outstanding',
    ...
    showingWrittenOff
      ? { key: 'on', label: 'Written Off On', width: '8%', render: (r) => shortDate(r.loan.writeoff_at) }
      : { key: 'on', label: 'Declared On', width: '8%', render: (r) => shortDate(r.loan.bad_debt_declared_at) },
    {
      key: 'why',
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Loan Writeoff" columns={columns} rows={filtered} />}>Loan Writeoff</MisPageTitle>
      ...
      >
        ...
        <Field label="Show">
          <select value={view} onChange={(e) => setView(e.target.value as 'pending' | 'written-off')} className="form-field">
      ...
      <MisModal open={!!active} onClose={() => setActive(null)} title="Write Off Loan" width="max-w-md">
        {active && (
          <div className="space-y-4 text-xs">
            ...
            <div className="flex justify-between rounded border border-red-200 bg-red-50 p-3">
              <span className="text-[10px] font-bold uppercase tracking-wide text-red-800">Amount to write off</span>
              <span className="text-sm font-bold text-chetu-red">{money(active.loan.outstanding_balance)}</span>
            </div>
            ...
            <p className="rounded border border-amber-200 bg-amber-50 p-2.5 text-amber-800">
              This closes the loan permanently and removes the balance from the portfolio. Only another Administrator
              can reopen it.
            </p>
