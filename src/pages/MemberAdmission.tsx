      ...
      clientGroups.filter(
        (g) =>
          ...
          (!form.loan_officer_id ||
            !g.loan_officer_id ||
            g.loan_officer_id === form.loan_officer_id),
      ),
    ...
    try {
      ...
      if (created.approval_status === "Pending") {
        addToast(
          "success",
          "Sent for Approval",
          `${created.full_name} (${created.client_number}) was submitted to your Branch Manager for approval.`,
        );
        ...
        addToast(
          "success",
          "Member Admitted",
          `Registered ${created.full_name} (${created.client_number})`,
        );
        navigate("/clients");
      }
    } catch (err: any) {
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            ...
            {branchLocked ? (
              ...
              <select
                ...
                onChange={(e) =>
                  set({ branch_id: e.target.value, group_id: "", loan_officer_id: "" })
                }
                className={field}
              >
          ...
          <div>
            ...
            <input
              value={form.voter_id}
              onChange={(e) => set({ voter_id: e.target.value })}
              className={field}
            />
          </div>
          <div>
        ...
        <div className="mt-4">
          ...
          >
            {docPreview ? (
              <img
                src={docPreview}
                alt="Uploaded member document preview"
        ...
        {/* Address Information */}
        <div className="form-section-title mt-6">Address Information</div>
        <div className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
          <div>
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-3">
          <div>
            ...
            <input
              readOnly
              value={FEES.admissionFee.toLocaleString()}
          ...
          <div>
            ...
            <input
              readOnly
              value={FEES.passbookFee.toLocaleString()}
              className={`${field} bg-slate-100`}
            />
          </div>
          <div>
            ...
            <input
              readOnly
              value={admissionFees().total.toLocaleString()}
        ...
        <p className="mt-2 text-[11px] text-slate-500">
          ...
          {FEES.securityDepositPct}%) are charged later, when the member takes a loan.
        </p>
        ...
        <button type="submit" disabled={saving} className="btn-save mt-6">
          {saving ? "Saving…" : "Save"}
        </button>
      </form>
    </div>
