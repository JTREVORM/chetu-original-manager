...
export const MemberListBase: React.FC<{
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q) {
        const hay =
          `${r.group_name} ${r.group_code} ${r.client.full_name} ${r.client.client_number} ${r.client.phone_number}`.toLowerCase();
  ...
  const columns: MisColumn<MemberRow>[] = [
    ...
    },
    {
      ...
      label: "Phone Number",
      width: "11%",
      render: (r) => r.client.phone_number,
      text: (r) => r.client.phone_number,
    },
    ...extraColumns,
    {
      key: "action",
  ...
  return (
    <div className="space-y-4 pb-16">
      ...
      <LoanHistoryModal
        row={historyFor}
        loans={historyFor ? memberLoans(historyFor.client.id) : []}
...
export const LoanDetailsInfoModal: React.FC<{
  row: MemberRow | null;
  loans: Loan[];
  onClose: () => void;
}> = ({ row, loans, onClose }) => {
        ...
        <table className="w-full min-w-[640px] table-fixed text-left text-[11px]">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {loans.map((l) => (
              <tr
                key={l.id}
                className="[&>td]:overflow-hidden [&>td]:text-ellipsis [&>td]:whitespace-nowrap [&>td]:px-2 [&>td]:py-2"
              >
...
export const LoanHistoryModal: React.FC<{
  row: MemberRow | null;
  loans: Loan[];
  onClose: () => void;
}> = ({ row, loans, onClose }) => {
...
export const FinancialFileUploadModal: React.FC<{ row: MemberRow | null; onClose: () => void }> = ({
  row,
  onClose,
}) => {
  const { addToast } = useNotifications();
    ...
    <MisModal open onClose={onClose} title="Member Bank Statement" width="max-w-2xl">
      ...
      <label className="mt-3 flex cursor-pointer flex-col items-center justify-center rounded border border-dashed border-slate-300 py-8 text-center hover:bg-slate-50">
        ...
        <span className="text-[11px] text-slate-500">
          {fileData ? "File ready to submit" : "Click to upload or drag and drop"}
        </span>
      ...
      <div className="mt-4 flex justify-center gap-3">
        <button
          type="button"
          onClick={onClose}
          className="rounded bg-chetu-red px-6 py-2 text-[12px] font-bold text-white"
        >
          No
        </button>
        <button
...
export const EditBusinessInfoModal: React.FC<{
  ...
  return (
    <MisModal
      open
      onClose={onClose}
      title={`Business Information — ${row.client.full_name}`}
      width="max-w-xl"
    >
      <p className="mb-3 text-[11px] text-slate-500">
        Only the business information, family guarantor and group guarantor details can be edited
        here.
      </p>
...
export const MemberList: React.FC = () => (
  <MemberListBase title="Member List" statuses={["Active"]} />
);
