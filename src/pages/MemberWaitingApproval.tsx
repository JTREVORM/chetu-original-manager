  ...
  const pendingClients = useMemo(() => {
    ...
    return clients
      ...
      .filter((c) => {
        ...
        const hay = `${c.full_name} ${c.client_number} ${group?.group_name || ''} ${group?.group_code || ''}`.toLowerCase();
    ...
    <div className="space-y-5 pb-12">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs">
              ...
              {pendingClients.map((c) => {
                ...
                return (
                  <tr key={c.id} className="transition-colors hover:bg-slate-50">
                    <td className="truncate px-2 py-3 text-slate-700">{group ? `${group.group_name} (${group.group_code})` : '—'}</td>
                    <td className="truncate px-2 py-3 font-bold text-[#0B4394]">{c.client_number}</td>
        ...
        <div className="divide-y divide-slate-100 md:hidden">
          {pendingClients.length === 0 && (
            <p className="p-8 text-center text-xs text-slate-400">No members waiting for approval.</p>
          )}
          {pendingClients.map((c) => {
            const group = c.group_id ? groupById.get(c.group_id) : undefined;
            return (
              <div key={c.id} className="space-y-2 p-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-600">
                  <p>Group: <strong className="text-slate-900">{group?.group_name || '—'}</strong></p>
                  <p>Phone: <strong className="text-slate-900">{c.phone_number}</strong></p>
                </div>
                <div className="flex items-center gap-1.5 pt-1">
                  {isLoanOfficer && (
                    <button onClick={() => openEdit(c)} className="rounded-lg bg-blue-50 p-1.5 text-[#0B4394]">
                      <Pencil className="h-4 w-4" />
                    </button>
                  )}
                  {canReview && (
                    <>
                      <button disabled={busyId === c.id} onClick={() => handleApprove(c)} className="rounded-lg bg-green-100 p-1.5 text-green-700 disabled:opacity-50">
                        <Check className="h-4 w-4" />
                      </button>
                      <button disabled={busyId === c.id} onClick={() => openReject(c)} className="rounded-lg bg-red-100 p-1.5 text-red-700 disabled:opacity-50">
                        <X className="h-4 w-4" />
                      </button>
                    </>
                  )}
          ...
          <div className="max-h-[90vh] w-[calc(100vw-1.5rem)] max-w-lg overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl md:w-full">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-3.5 p-4 text-xs md:p-6">
              ...
              <div className="flex flex-col-reverse gap-2 border-t pt-4 md:flex-row md:justify-end">
                <button type="button" onClick={() => setEditingClient(null)} className="rounded-xl bg-slate-100 px-4 py-2 font-bold text-slate-700">
                  Cancel
                </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs">
          <div className="w-[calc(100vw-1.5rem)] max-w-md overflow-y-auto rounded-3xl border border-slate-200 bg-white shadow-2xl md:w-full">
            <div className="flex items-center justify-between bg-red-600 p-5 text-white">
              ...
              <button onClick={() => setRejectingClient(null)} className="text-white/80 hover:text-white">
                <X className="h-5 w-5" />
              </button>
            </div>
            <form onSubmit={handleRejectSubmit} className="space-y-4 p-6 text-xs">
              <p className="text-slate-600">
                Rejecting <strong className="text-slate-900">{rejectingClient.full_name}</strong> ({rejectingClient.client_number}). The Loan Officer will be able to edit and resubmit.
              </p>
              <div>
              ...
              <div className="flex flex-col-reverse gap-2 border-t pt-3 md:flex-row md:justify-end">
                <button type="button" onClick={() => setRejectingClient(null)} className="rounded-xl bg-slate-100 px-4 py-2 font-bold text-slate-700">
                  Cancel
                </button>
                <button type="submit" disabled={busyId === rejectingClient.id} className="rounded-xl bg-red-600 px-5 py-2 font-black text-white shadow-md hover:bg-red-700 disabled:opacity-50">
                  Confirm Rejection
                </button>
              </div>
            </form>
