...
import { Receipt, Search, Plus, Printer, CheckCircle2, Clock, Calendar, ChevronRight, X } from 'lucide-react';
import { PageHeader, FilterBar, DesktopOnly, MobileOnly, RecordCard, CardList, EmptyState } from '../components/mobile/Responsive';
...
export const Repayments: React.FC = () => {
  ...
  const activeLoans = loans.filter(l => l.status === 'Active' || l.status === 'Partially Paid' || l.status === 'Overdue');
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const repaymentRecord = await recordRepayment(selectedLoan.id, paymentAmount, paymentMethod, notes);
      addToast('success', 'Payment Recorded', `Receipt #${repaymentRecord.receipt_number} generated for ${formatUGX(paymentAmount)}`);
      ...
      try {
        ...
        addToast('warning', 'Receipt Not Printed', 'Payment saved, but the receipt PDF could not be generated.');
      }
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-card overflow-hidden">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          ...
          <span className="text-xs font-semibold text-slate-500">{filteredLoans.length} Loans Found</span>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {filteredLoans.map((loan) => (
                <tr key={loan.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4 font-bold text-red-600">{formatUGX(loan.outstanding_balance)}</td>
                  <td className="p-4 font-bold text-slate-900">{formatUGX(loan.weekly_installment)}</td>
                  <td className="p-4">
                    ...
                    <span className="text-[10px] text-slate-500 mt-0.5 block">{loan.completion_percentage}% Paid</span>
                  </td>
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                      loan.status === 'Active' ? 'bg-blue-100 text-blue-800' :
                      loan.status === 'Partially Paid' ? 'bg-amber-100 text-amber-800' :
                      loan.status === 'Fully Paid' ? 'bg-emerald-100 text-emerald-800' :
              ...
              {filteredLoans.length === 0 && (
                <tr>
                  <td colSpan={isAuditor || isBranchManager ? 7 : 8} className="p-8 text-center text-xs text-slate-400 font-medium">
              ...
              {filteredLoans.map((loan) => (
                <RecordCard
                  ...
                  fields={[
                    ...
                    { label: 'Outstanding Bal', value: <span className="text-red-600">{formatUGX(loan.outstanding_balance)}</span> },
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-card">
        ...
        <div className="space-y-2">
          {repayments.slice(0, 5).map((rep) => (
            <div key={rep.id} className="p-3 bg-slate-50 rounded-xl border border-slate-100 flex items-center justify-between text-xs">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-emerald-100 text-emerald-800 rounded-lg font-bold text-xs">
                  <Receipt className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="font-bold text-slate-900">{rep.receipt_number} ({rep.client?.full_name})</h4>
                  <p className="text-slate-500 text-[11px]">{rep.payment_date} • Method: {rep.payment_method}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="font-black text-emerald-700 text-sm">{formatUGX(rep.amount_paid)}</span>
                <button
          ...
          <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-[calc(100vw-1.5rem)] max-w-md md:w-full overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handlePaymentSubmit} className="p-4 md:p-6 space-y-4">
              <div className="p-3 bg-blue-50 rounded-xl border border-blue-100 text-xs space-y-1">
                <p><strong>Borrower:</strong> {selectedLoan.client?.full_name}</p>
                <p><strong>Current Outstanding Balance:</strong> {formatUGX(selectedLoan.outstanding_balance)}</p>
                <p><strong>Standard Weekly Installment:</strong> {formatUGX(selectedLoan.weekly_installment)}</p>
              </div>
