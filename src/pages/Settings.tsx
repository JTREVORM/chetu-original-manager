  ...
  const handleReset = async () => {
    ...
    try {
      ...
      addToast(
        "success",
        "System reset",
        "All operational data has been cleared. Signing you out.",
      );
      setTimeout(() => {
    ...
    <div className="space-y-4 pb-16">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-4 p-4 sm:p-5">
            ...
            <Hint>
              ...
              <button
                ...
                className="font-semibold text-[#0B4394] underline"
              >
                Loan Products
              </button>
              , not here — each product keeps the rate it was saved with. Loan charges are fixed by
        ...
        <Panel title="Fee schedule">
          <div className="p-4 sm:p-5">
            <dl className="grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
              <FeeRow
                label="Admission fee"
                value={`UGX ${FEES.admissionFee.toLocaleString()}`}
                note="once per member"
              />
              <FeeRow
                label="Passbook fee"
                value={`UGX ${FEES.passbookFee.toLocaleString()}`}
                note="once per member"
              />
              <FeeRow
                label="Loan processing fee"
                value={`${FEES.processingFeePct}%`}
                note="of principal"
              />
              <FeeRow label="CRB fee" value={`${FEES.crbFeePct}%`} note="of principal" />
              <FeeRow
                label="Security deposit"
                value={`${FEES.securityDepositPct}%`}
                note="of principal, refundable"
              />
              <FeeRow
                label="Group maintenance"
                value={`UGX ${FEES.groupMaintenanceFee.toLocaleString()}`}
                note="per loan"
              />
            </dl>
            <Hint>
              Charges are stored on each loan when it is approved, so a loan always reports what it
              was actually charged. Changing the schedule affects new loans only and is a code
        ...
        <Panel title="Document text">
          <div className="space-y-4 p-4 sm:p-5">
            <div>
              <label className="form-label" htmlFor="report_header">
                Report header
              </label>
              <input
                id="report_header"
            ...
            <div>
              <label className="form-label" htmlFor="receipt_footer">
                Receipt footer
              </label>
              <textarea
                id="receipt_footer"
        ...
        <Panel title="Related modules">
          <div className="grid grid-cols-1 divide-y divide-slate-100 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            <LinkRow
              ...
              desc="Create branches and attach staff."
              onClick={() => navigate("/branches")}
            />
            <LinkRow
              icon={UserCog}
              title="User Management"
              desc="Staff accounts, roles and branches."
              onClick={() => navigate("/users")}
            />
            <LinkRow
              icon={FileText}
              title="Audit Logs"
              desc="Every action recorded, by whom and when."
  ...
  <div className="rounded-lg border border-slate-200 bg-white px-4 py-3.5 shadow-xs sm:px-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0">
        ...
        {subtitle && (
          <p className="mt-0.5 max-w-2xl text-[13px] leading-relaxed text-slate-500">{subtitle}</p>
        )}
      </div>
...
const FeeRow: React.FC<{ label: string; value: string; note: string }> = ({
  label,
  value,
  note,
}) => (
...
const LinkRow: React.FC<{
  ...
  <button
    type="button"
    onClick={onClick}
    className="flex items-start gap-3 p-4 text-left hover:bg-slate-50"
  >
