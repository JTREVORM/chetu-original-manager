...
export const OutstandingReport: React.FC = () => {
  ...
  const filtered = useMemo<Row[]>(() => {
    ...
    for (const p of repayments) {
      ...
      if ((p.collection_type || "").toLowerCase() === "migration")
        bucket.migration += Number(p.amount_paid);
  ...
  const columns: MisColumn<Row>[] = [
    ...
    {
      key: "mig",
      label: "Migration Collection",
      width: "9%",
      align: "right",
      render: (r) => money(r.migrationCollection),
    },
    {
      key: "run",
      ...
      width: "9%",
      align: "right",
      render: (r) => money(r.runningCollection),
    },
    {
      key: "tot",
      ...
      width: "9%",
      align: "right",
      render: (r) => money(r.totalCollection),
    },
    {
      key: "out",
      ...
      align: "right",
      render: (r) => (
        <span className="font-bold text-[#0B4394]">{money(r.loan.outstanding_balance)}</span>
      ),
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle
        right={<ReportExportButtons title="Outstanding Report" columns={columns} rows={filtered} />}
      >
        Outstanding Report
      </MisPageTitle>
      ...
      <MisTable
        ...
        mobileSubtitle={(r) =>
          `${r.loan.loan_number} • Outstanding ${money(r.loan.outstanding_balance)}`
        }
        footer={
          filtered.length > 0 && (
