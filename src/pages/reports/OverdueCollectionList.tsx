...
export const OverdueCollectionList: React.FC = () => {
  ...
  const filtered = useMemo<Row[]>(() => {
    ...
    return repayments
      ...
      .map<Row | null>((p) => {
        ...
        return { id: p.id, base, collectionDate: p.payment_date, collectionAmount: Number(p.amount_paid) };
      })
  ...
  const columns: MisColumn<Row>[] = [
    { key: 'date', label: 'Collection date', width: '10%', render: (r) => shortDate(r.collectionDate) },
    ...
    },
    {
      ...
      width: '10%',
      render: (r) => (
        <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-700">{r.base.loan.status}</span>
      ),
        ...
        footer={
          filtered.length > 0 && (
            <div className="flex items-center justify-end px-4 py-3 text-xs font-bold text-slate-700">
              Total Overdue Collection: <span className="ml-2 text-emerald-600">{money(total)}</span>
            </div>
          )
        }
      />
