...
export const GroupInterchange: React.FC = () => {
  ...
  const confirmMove = async () => {
    ...
    try {
      ...
      addToast('success', 'Member moved', `${active.client.full_name} is now in ${to?.group_name || 'the new group'}.`);
      setActive(null);
    } catch (error) {
      addToast('error', 'Move failed', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const columns: MisColumn<Row>[] = [
    ...
    { key: 'lo', label: 'LO', width: '11%', render: (r) => r.officer_name, text: (r) => r.officer_name },
    { key: 'code', label: 'Member Code', width: '11%', render: (r) => r.client.client_number, text: (r) => r.client.client_number },
    {
      key: 'name',
      label: 'Member Name',
      width: '18%',
    ...
    { key: 'grp', label: 'Current Group', width: '15%', render: (r) => r.group_name, text: (r) => r.group_name },
    { key: 'gcode', label: 'Group Code', width: '12%', render: (r) => r.group_code, text: (r) => r.group_code },
    { key: 'joined', label: 'Registered', width: '10%', render: (r) => shortDate(r.client.date_registered) },
    {
      key: 'act',
      label: 'Action',
      width: '6%',
      ...
      render: (r) =>
        canMove ? (
          <ActionButton
            onClick={() => { setActive(r); setToGroupId(''); setReason(''); }}
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Group Interchange" columns={columns} rows={filtered} />}>Group Interchange</MisPageTitle>
      ...
      <MisModal open={!!active} onClose={() => setActive(null)} title="Move Member to Another Group" width="max-w-md">
        {active && (
          <div className="space-y-4 text-xs">
            <div className="rounded border border-slate-200 bg-slate-50 p-3">
              <p className="text-sm font-bold text-slate-900">{active.client.full_name}</p>
              <p className="text-slate-500">
                {active.client.client_number} • currently in {active.group_name} • {active.branch_name}
              </p>
            </div>
            ...
            {destinations.length === 0 && (
              <p className="rounded border border-amber-200 bg-amber-50 p-2.5 text-amber-800">
                There is no other approved active group in {active.branch_name}. To move this member to a different
                branch, use Member Branch Transfer.
              </p>
            )}
            ...
            <p className="rounded border border-slate-200 bg-slate-50 p-2.5 text-slate-600">
              The member takes on the new group's loan officer. Any open loans stay with the member and follow them.
            </p>
