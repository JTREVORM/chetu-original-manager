...
export const MemberBranchTransfer: React.FC = () => {
  ...
  const { clients, clientGroups, loans, branches, transfers, requestMemberTransfer, cancelMemberTransfer } = useDatabase();
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    const pendingByClient = new Set(
      transfers.filter((t) => t.status === 'Pending' && t.client_id).map((t) => t.client_id as string),
    );
    ...
    return clients
      ...
      .map((c) => {
        ...
        const open = loans.filter(
          (l) => l.client_id === c.id && !CLOSED_LOAN_STATUSES.includes(l.status) && l.status !== 'Pending',
        );
        return {
          client: c,
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q && !`${r.group_name} ${r.client.full_name} ${r.client.client_number}`.toLowerCase().includes(q)) return false;
      return true;
    });
  ...
  const confirmSend = async () => {
    ...
    try {
      ...
      addToast('error', 'Transfer failed', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const withdraw = async (row: Row) => {
    ...
    try {
      ...
      addToast('error', 'Could not withdraw', error instanceof Error ? error.message : 'Please try again.');
    }
  };
  ...
  const columns: MisColumn<Row>[] = [
    ...
    { key: 'code', label: 'Member Code', width: '10%', render: (r) => r.client.client_number, text: (r) => r.client.client_number },
    {
      key: 'name',
      label: 'Member Name',
      width: '15%',
    ...
    { key: 'joined', label: 'Registered', width: '9%', render: (r) => shortDate(r.client.date_registered) },
    { key: 'loans', label: 'Open Loans', width: '7%', align: 'center', render: (r) => r.open_loans },
    {
      key: 'out',
      label: 'Outstanding',
      width: '10%',
      align: 'right',
      render: (r) => (r.outstanding > 0 ? <span className="font-bold text-chetu-red">{money(r.outstanding)}</span> : '—'),
    ...
    },
    {
      ...
      width: '9%',
      render: (r) =>
        r.pending_transfer ? <span className="font-bold text-amber-600">Awaiting receipt</span> : 'In branch',
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Member Branch Transfer" columns={columns} rows={filtered} />}>Member Branch Transfer</MisPageTitle>
      ...
      <MisModal open={!!active} onClose={() => setActive(null)} title="Transfer Member to Another Branch" width="max-w-lg">
        {active && (
          <div className="space-y-4 text-xs">
            ...
            {active.open_loans > 0 && (
              <div className="rounded border border-amber-200 bg-amber-50 p-3 text-amber-800">
                <p className="font-bold">
                  {active.open_loans} open loan{active.open_loans === 1 ? '' : 's'} — {money(active.outstanding)} outstanding
                </p>
                <p className="mt-0.5">
                  The loans move with the member and the receiving branch takes over collections. Settle first if that is
                  not what you intend.
                </p>
              </div>
            )}
            ...
            <Field label="Receiving Branch *">
              <select value={toBranchId} onChange={(e) => setToBranchId(e.target.value)} className="form-field">
            ...
            <p className="rounded border border-slate-200 bg-slate-50 p-2.5 text-slate-600">
              The member stays in this branch until a Branch Manager at the receiving branch accepts them. They pick the
              destination group and officer at that point.
            </p>
