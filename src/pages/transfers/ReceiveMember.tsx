...
export const ReceiveMember: React.FC = () => {
  ...
  const {
    transfers, clients, clientGroups, branches,
    receiveMemberTransfer, rejectMemberTransfer,
  } = useDatabase();
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (q && !`${r.member_name} ${r.member_code} ${r.from_branch}`.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [rows, applied]);
  ...
  const destinationOfficers = useMemo(
    () => scope.officers.filter((o) => (o.branch_ids || []).includes(accepting?.transfer.to_branch_id || '')),
  ...
  const confirmAccept = async () => {
    ...
    setBusy(true);
    try {
      await receiveMemberTransfer(accepting.transfer.id, toGroupId || undefined, toOfficerId || undefined);
      addToast('success', 'Member received', `${accepting.member_name} now belongs to ${accepting.to_branch}.`);
      setAccepting(null);
    } catch (error) {
      addToast('error', 'Could not receive', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const confirmReject = async () => {
    ...
    try {
      ...
      addToast('info', 'Transfer rejected', `${rejecting.member_name} stays in ${rejecting.from_branch}.`);
      setRejecting(null);
      setReason('');
    } catch (error) {
      addToast('error', 'Could not reject', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setBusy(false);
    }
  };
  ...
  const columns: MisColumn<Row>[] = [
    { key: 'code', label: 'Member Code', width: '11%', render: (r) => r.member_code, text: (r) => r.member_code },
    {
      key: 'name',
      label: 'Member Name',
      width: '16%',
    ...
    { key: 'from', label: 'From Branch', width: '12%', render: (r) => r.from_branch, text: (r) => r.from_branch },
    { key: 'fromgrp', label: 'From Group', width: '12%', render: (r) => r.from_group, text: (r) => r.from_group },
    { key: 'to', label: 'To Branch', width: '12%', render: (r) => r.to_branch, text: (r) => r.to_branch },
    { key: 'on', label: 'Sent On', width: '9%', render: (r) => shortDate(r.requested_on) },
    { key: 'by', label: 'Sent By', width: '11%', render: (r) => r.requested_by, text: (r) => r.requested_by },
    {
      key: 'why',
      label: 'Reason',
      width: '11%',
    ...
    },
    {
      ...
      render: (r) => {
        if (r.transfer.status !== 'Pending') {
          return (
            <span className={`text-[10px] font-bold ${r.transfer.status === 'Completed' ? 'text-emerald-700' : 'text-chetu-red'}`}>
        ...
        return (
          <div className="flex items-center justify-center gap-1">
            ...
            <ActionButton onClick={() => { setRejecting(r); setReason(''); }} title="Reject this transfer" tone="red">
  ...
  return (
    <div className="space-y-4 pb-16">
      <MisPageTitle right={<ReportExportButtons title="Receive Member" columns={columns} rows={filtered} />}>Receive Member</MisPageTitle>

      {!canAction && (
        <p className="rounded border border-slate-200 bg-slate-50 p-2.5 text-[11px] text-slate-500">
          Receiving a member is done by a Branch Manager of the receiving branch. You can review incoming transfers here.
        </p>
      )}
      ...
      <MisModal open={!!accepting} onClose={() => setAccepting(null)} title="Receive Member" width="max-w-lg">
        {accepting && (
          <div className="space-y-4 text-xs">
            ...
            <Field label="Place in Group">
              <select value={toGroupId} onChange={(e) => setToGroupId(e.target.value)} className="form-field">
            ...
            {destinationGroups.length === 0 && (
              <p className="rounded border border-amber-200 bg-amber-50 p-2.5 text-amber-800">
                This branch has no approved active groups yet. You can still receive the member and place them later.
              </p>
            )}
            ...
            <p className="rounded border border-slate-200 bg-slate-50 p-2.5 text-slate-600">
              Accepting moves the member and any open loans into your branch. Collections become your branch's
              responsibility from that moment.
            </p>

            <div className="flex flex-col-reverse gap-2 border-t border-slate-200 pt-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setAccepting(null)} className="rounded bg-slate-100 px-4 py-2 font-bold text-slate-700">
                Cancel
              </button>
              <button
                type="button"
      ...
      <MisModal open={!!rejecting} onClose={() => setRejecting(null)} title="Reject Transfer" width="max-w-md">
        {rejecting && (
          <div className="space-y-4 text-xs">
            <p className="text-slate-600">
              Rejecting sends <strong className="text-slate-900">{rejecting.member_name}</strong> back to{' '}
              <strong className="text-slate-900">{rejecting.from_branch}</strong>. Nothing about the member changes.
            </p>
            <Field label="Reason *">
              <textarea
                rows={3}
            ...
            <div className="flex flex-col-reverse gap-2 border-t border-slate-200 pt-3 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setRejecting(null)} className="rounded bg-slate-100 px-4 py-2 font-bold text-slate-700">
                Cancel
              </button>
              <button
                type="button"
